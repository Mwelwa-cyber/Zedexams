...
test('aiGenerations client create stays locked to the client-side tools', () => {
  ...
  assert(
    /incoming\(\)\.tool in \['mark_schedule', 'weekly_forecast', 'record_of_work', 'class_timetable'\]/.test(create[0]),
    "aiGenerations create tool list changed — only the client-side (non-AI) tools mark_schedule + weekly_forecast + record_of_work + class_timetable may be client-created; AI tools must stay server-created so cost/token fields cannot be forged",
  )
