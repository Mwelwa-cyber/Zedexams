...

{
  ...
  // two recorded edges must leave the third visible rather than swallowing it.
  const graph = buildGraph({
  ...
  assert(findPath(withoutEdge(g, 'Editor', 'heavy'), 'Page-11111111.js', 'heavy') === null,
    'cutting every route leaves nothing — which is what a complete record looks like');
}
