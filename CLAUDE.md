...
src/
  ...
  components/
    ...
    quiz/                       — the quiz AUTHORING surface only, as of 2026-08-14: EditQuizV2, QuizSectionsEditor, the document + scanned-image (OCR) importers, the rich-text/format/parse modules, QuizVerifyModal (calls Vex). The learner-facing RUNTIME (QuizList, QuizRunnerV2, QuizResultsV2) moved to src/features/quiz/ — see below. This half stays because frozen `admin/CreateQuizV2` imports five of its modules, whose closure inside the directory is 38 modules; it becomes `features/quizEditor` when CreateQuizV2 is released. Six one-line re-export shims under `quiz/{reading,review}/` + `quiz/ExtraQuestionImages.jsx` keep the frozen DailyExamRunner resolving. Question types include MCQ, short-answer, and Fill-in-the-Blanks
  ...
  features/games — the games surface (`/games`): hub, subject selector, game list, play surface, global leaderboard, and the eight game engines. Migrated out of `src/components/games/` on 2026-08-14 by an explicit **owner ruling releasing it from the Assessment Engine freeze** — the third such ruling, and like the other two it releases ONE surface: the rollout flags are still at 0 and `components/quiz/`, `PastPaperStudio`, `CreateQuizV2`, `GamesSeedAdmin` and `DailyExamRunner` all stay frozen. `TimedQuizGame` is the engine's LAST cutover; its `useAssessmentEngineFlag('game')` wiring and both its specs (`.engine.spec` / `.legacy.spec`) moved unchanged and are load-bearing. The front door exports two names only — `GameBadgeCard` and `getSubjectMascot`. **`GameStickerStyles` and `Confetti` are NOT in the feature**: they went to `src/shared/components/`, because exporting them from the front door cost +75 kB of static edges each on GradeHub, SubjectDrillDown and ExamResultsPage (the door also exports `GameBadgeCard`, which reaches `utils/gamesService` → Firebase). `src/utils/gamesService.js`, `gameBadgesService.js`, `gameProgress.js`, `gameScorePayload.js` and `gamesIntelligence.js` deliberately stayed behind — each has a consumer outside the feature, and `gamesService`'s is the frozen `GamesSeedAdmin`, which is also why the feature has no `services/` directory and §14.2 is knowingly unsatisfied. Only `gameSounds.js` travelled, into `lib/`. `QuizList` reached the sticker stylesheet through a one-line re-export shim at the old path while `components/quiz/` was still frozen; the quiz migration hours later moved QuizList, so the shim retired and `src/components/games/` is gone
  features/quiz — the learner-facing quiz RUNTIME (`/quizzes`, `/quiz/:quizId`, `/quiz/:quizId/results`): QuizList, QuizRunnerV2, QuizResultsV2, QuizTip. Migrated out of `src/components/quiz/` on 2026-08-14 by an explicit **owner ruling releasing that directory from the Assessment Engine freeze** — the fourth such ruling. **It released the directory, not everything the directory serves:** `admin/CreateQuizV2` is its own freeze entry and pins 38 modules of quiz AUTHORING, which stay in `components/quiz/` and become `features/quizEditor` when it is released. The split was measured first — the runner's closure is 12 modules, the editor's 29, and they share exactly two (`ExtraQuestionImages`, `ZoomableImage`), both of which went to `src/shared/components/` on consumer count. **Empty front door** — all three screens are route-mounted lazily and nothing else imports them. `QuizRunnerV2` carries the Phase 3 `useAssessmentEngineFlag('quiz')` branch and ALL THREE specs travelled with it, including `.characterisation.spec.jsx`, the recorded byte-compatibility baseline. The reading-assist cluster (PassageViewer, ReadingSettings*, TextToSpeechButton, ThemePreview) + QuizReviewScreen also went to `src/shared/components/` — three areas draw each of them; frozen `DailyExamRunner` reaches six through one-line shims at the old paths and stays byte-identical
  features/papers — the past-paper archive (`/papers`): hub, reader, timed practice, the public paper-quiz runner, learner history. Migrated out of `src/components/papers/` on 2026-08-13 by an explicit **owner ruling releasing it from the Assessment Engine freeze** — the rollout flags are still at 0, so the rest of that freeze (quizzes, games, `PastPaperStudio`, `CreateQuizV2`) still stands; do not read this as the freeze having lifted. `PublicQuizRunner` is the engine's designated FIRST flag flip, so its `useAssessmentEngineFlag('pastPaperQuiz')` wiring and its zero-write property (`test:paper-quiz-zero-write`) are load-bearing. The front door exports only `PaperTitle`/`PaperSourceBadge`; the PDF viewers (`PdfJsViewer`, `PdfScrollViewer`, + their chrome) are NOT papers' — they went to `src/shared/components/`, because `PdfScrollViewer`'s only consumer is the exam-timetable page. `src/utils/pastPapers.js` and `pastPaperQuizStatus.js` deliberately stayed behind: their other consumers are still frozen
...
**The automatic security scanner is CodeQL** (`codeql.yml`, added 2026-08), not the AI reviewer. It is free because this repository is public — as are Actions minutes — so it restored default coverage after the AI review went opt-in, at zero Anthropic spend. It scans `javascript-typescript` on every PR to `main` under the `security-extended` suite, and findings land in the **Security tab**. Three things to know before changing it:

- **It is not, and must not become, a required check.** Code scanning reports against the whole repository, so a pre-existing alert unrelated to a PR would block that PR forever.
- **There is deliberately no `push: [main]` trigger**, for the runner-contention reason `ci.yml` documents — a job on the merge commit makes the deploys queue behind it. A weekly cron baselines the default branch instead. The cost: for up to seven days an alert already on `main` can surface on a PR as if that PR introduced it, and one fixed on `main` stays open until the next scheduled run. Both self-correct and neither blocks anything.
- **Scope is guarded** by `test:codeql-scope`, because every way this breaks is silent — a source root added to `paths-ignore`, a `paths:` filter on the trigger, or a `config-file` pointing nowhere all yield a scan that reports "no new alerts" having examined nothing.

CodeQL and the AI reviewer are complementary rather than redundant: CodeQL follows taint across the whole codebase instead of reading one diff, but it cannot reason about authorization semantics ("this callable never checks the caller's role"). That gap is covered behaviourally by the Firestore/Storage rules emulator jobs, and the `security-review` label is the escape hatch when an AI opinion is actually wanted — label anything touching auth, Firestore rules, payments, user input, or a new external surface. The full agent roster (including departments + invocation modes) is the single source of truth in `src/config/agents.js`; keep it and [`ORG.md`](./ORG.md) in sync when you add one.
