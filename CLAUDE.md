...
**A retired model is a 404, not a downgrade — `test:retired-models` is what stops one shipping.** Model IDs are plain strings across ~40 files and 39 `*_MODEL` env overrides, so nothing in lint or the type system can tell a live ID from a retired one; the failure surfaces in production as `not_found_error` on the surfaces a user is waiting on (Vex's verify, Qix's review, Bonga's replies, paper explanations). `scripts/test-retired-anthropic-models.mjs` scans every tracked source, config and doc for three things: an exactly-retired ID, any pre-4.x `claude-<digit>…` / `claude-instant…` string (that whole generation is retired or retiring, and a `-latest` alias resolves to a retired snapshot while reading as current), and any versioned Claude family ID absent from its `KNOWN_MODELS` set. **Adding a model means editing that set** — which is the deliberate "is this one still live?" check, and the reason the guard is an allowlist rather than only a deny-list. Prose may name a retired model; a line needing the literal ID opts out with the marker `retired-model-ok`. Anthropic retired Haiku 3.5 and Sonnet 3.7 on 2026-02-19; this repo was already clear of both, and the guard is what makes that a fact rather than a coincidence.

**A model is switched by setting, not by editing call sites — `anthropicRequestPolicy.js` is why that is safe.** Newer models refuse fields older ones accept: Sonnet 5, Opus 4.7+ and every 5-generation model answer a non-default `temperature`/`top_p`/`top_k` with a 400, and nearly every call here sends one (`callClaude` defaults `temperature` to 0.3). Sonnet 5 also runs adaptive thinking when `thinking` is omitted, where 4.x ran thinking-off. `anthropicFetch` — the one path every Messages request takes — shapes each body for its model: sampling is dropped where refused, and Sonnet 5's thinking is pinned `{type: "disabled"}` unless the caller set it. A model the policy does not recognise goes out byte-identical. So moving a surface to Sonnet 5 is `ANTHROPIC_MODEL=claude-sonnet-5` (or the per-tool `*_MODEL`), and rolling back is unsetting it. **Call Anthropic through `anthropicFetch`, never a bare `fetch`** — `test:anthropic-request-policy` fails on a file that sends an `anthropic-version` header without it, because a direct caller would bypass the shaping and 400 on the new model.
...
- **ESLint config is in `eslint.config.js`** (flat config). The file is heavily commented with the rationale for each rule. `no-unused-vars` exempts `caughtErrors` (lots of intentional `catch (err) {}`), `no-empty` allows empty catches, `eqeqeq` is `'smart'`. Tests + Cloud Functions get Node globals and `no-unused-vars` off. **`no-undef` is ON for `functions/`** (2026-09): it was off, and three live callables shipped a `ReferenceError` past a green CI — including the Lenco payment-status poll — because an identifier imported inside one handler was used in another.
...
- **Backend code uses firebase-admin's MODULAR imports, never the namespace API** (migrated 2026-09, #2666 → #2668 → #2670, then the v14 bump). firebase-admin 14 deletes `admin.firestore()` / `admin.auth()` / `admin.storage()` / `admin.messaging()`, the statics on them (`admin.firestore.FieldValue`, `.Timestamp`, `.FieldPath`) and `admin.apps`; on 14 those are `undefined is not a function` at RUNTIME, which neither lint nor most tests would see. Write `const {getFirestore, FieldValue} = require("firebase-admin/firestore")` (and `firebase-admin/auth`, `/storage`, `/messaging`, `/app-check`, `/app` for `initializeApp`/`getApps`). **Every file in `functions/` is now on the modular API** and `test:firebase-admin-modular` is a plain ban (its `LEGACY` list is empty). The handler builders `index.js` wires through `batch2HandlerDeps` / `batch3HandlerDeps` (`paymentHandlers`, `messagingHandlers`, `agentOpsHandlers`, `httpSurfaceHandlers`, `visualAiHandlers`) receive `getFirestore` / `FieldValue` / `Timestamp` / `getAuth` / `getStorage` by injection — never an `admin` namespace — so a test injects `getFirestore: () => fakeDb`. A test that fakes the SDK in the legacy `{firestore: fn, …}` shape serves the modular paths from the same fake via `functions/testFixtures/modularAdminStub.js`, which resolves at CALL time and hands out `FieldValue`-style statics as forwarding proxies, because production code destructures them at load while tests swap fakes per case. `functions/` is on **firebase-admin 14** (stage 4); the unused `firebase-functions-test` devDependency was removed for it, since its peer range stops at admin 13 — do not re-add it without checking that range. v14 also stops exporting `firebase-admin/package.json`, so nothing may `require` it for a version string. The repo-root `firebase-admin` (used by the `scripts/` operations and migration scripts) is a separate install, and it is on **14** too (2026-09). Scripts load the SDK through `scripts/lib/adminSdk.mjs` — `const admin = await loadAdminSdk()`, then `admin.getFirestore()`, `admin.FieldValue`, `admin.getApps()`, `admin.getAuth()`, `admin.getStorage()` — a lazy import of the four modular entry points, so dry runs and fixture modes still load nothing. `test:firebase-admin-modular` scans `scripts/` as well and also bans `admin.default.firestore()`, `admin.apps` and `admin.credential` there. Those scripts are run by hand against production and no CI job executes them, so the guard is the only thing between a namespace call and a script failing mid-run.
- **A scheduled function declares `maxInstances`; so do the admin-only callables** (2026-09). Every function here runs on 1 vCPU and, uncapped, may scale to 100 instances — so each can claim up to 100 vCPUs of the project's Cloud Run CPU quota in its region, and that quota is what failed functions deploys #811, #813 and #815 (`Quota exceeded for total allowable CPU per project per region`). A cron runs once per tick and one instance serves 80 concurrent requests, so all 41 `onSchedule` functions are `maxInstances: 1`; the 20 admin tools in `ADMIN_CAPPED` are `2`. `test:function-instance-caps` fails on an `onSchedule` without a cap (inline, via a named options constant, or via a spread) and on an admin cap that disappears. User-facing callables, webhooks and Firestore triggers are deliberately left at the default — capping a hot path turns a traffic spike into refused requests. The real headroom is a quota increase in the GCP console (Cloud Run Admin API → "Total CPU allocation" in us-central1); the caps only stop idle surfaces from counting against it.
