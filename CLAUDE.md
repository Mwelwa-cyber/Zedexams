...
- **Backend code uses firebase-admin's MODULAR imports, never the namespace API** (migration in progress, 2026-09). firebase-admin 14 deletes `admin.firestore()` / `admin.auth()` / `admin.storage()` / `admin.messaging()` and the statics on them (`admin.firestore.FieldValue`, `.Timestamp`, `.FieldPath`); on 14 those are `undefined is not a function` at RUNTIME, which neither lint nor most tests would see. Write `const {getFirestore, FieldValue} = require("firebase-admin/firestore")` (and `firebase-admin/auth`, `/storage`, `/messaging`, `/app-check`) — 13.x already supports it. `functions/` stays on 13.x until the shrink-only `LEGACY` list in `scripts/test-firebase-admin-modular.mjs` (`test:firebase-admin-modular`) is empty; that guard fails a NEW namespace use and a listed file that no longer needs listing. A test that fakes the SDK with a legacy-shaped `{firestore: fn, …}` stub serves the modular paths from the same stub via `functions/testFixtures/modularAdminStub.js`, so the two can't disagree. The `firebase-functions-test` devDependency in `functions/package.json` must go before the bump — its peer range stops at admin 13 and nothing imports it. The repo-root `firebase-admin` (used by `scripts/` migrations) is a separate install and not part of this.
