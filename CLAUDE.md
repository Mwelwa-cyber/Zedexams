...
- **A `permissions:` block that omits `actions` now 403s.** The Actions REST API serves run metadata for PUBLIC repositories without the `actions` scope, so `deploy-hosting.yml`'s "Wait for Deploy Firebase run" step polled it happily under `permissions: contents: read` for as long as the repo was public. The first deploy after the flip got `HTTP Error 403: Forbidden` on its first poll, which crashed the step in under a second and **skipped the Hosting deploy** — with lint, tests, build and prerender all green above it, so the run reads as a deploy that got as far as deploying. Production simply stopped receiving frontend changes on every commit touching `functions/` (the only commits that reach that step). Fixed by adding `actions: read`; `test:workflow-api-permissions` fails if any workflow job calling the Actions API loses that grant, and fails too if its detector stops matching. Worth generalising: **a declared `permissions:` block is exhaustive**, and the trigger here was a repository VISIBILITY change, not a diff — nothing in a code review could have caught it and re-running the job never would have fixed it.
