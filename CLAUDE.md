...
src/
  ...
  components/
    ...
    quiz/                       — the document-IMPORT body only, as of 2026-08-14: documentQuizImporter + parserCore/tableBlocks/paragraphRuns/reconcile, scannedQuizImporter (OCR), importFormatChecks, pastPaperParts. Everything else migrated — the RUNTIME to src/features/quiz/, the EDITOR to src/features/quizEditor/ (both below). This body stays because `src/utils/paperToQuizConverter.js` reads it and is pinned by the frozen `admin/AdminPastPapers`, and it cannot go down to shared/ either: its closure is 55 modules reaching eight Firebase entry points, and shared is in NO_FIREBASE_LAYERS. Six one-line re-export shims under `quiz/{reading,review}/` + `quiz/ExtraQuestionImages.jsx` keep the frozen DailyExamRunner resolving. Question types include MCQ, short-answer, and Fill-in-the-Blanks
