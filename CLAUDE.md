...
**⚠️ THE REPOSITORY IS PRIVATE (since 2026-08-18), and nothing below should be read as saying otherwise.** Text elsewhere in this file and in `codeql.yml` used to state it was public and that this was why code scanning and Actions minutes were free; that was true when written and is not any more. Two consequences that change what you should do:

- **Actions minutes are METERED.** One CI run bills ~36 minutes across its ten jobs (Vitest alone is 14 of them), and a PR push costs 44–73 minutes once the visual gate is included. At the historical cadence (~30 CI runs/day) that is tens of thousands of minutes a month against an allowance of 2,000–3,000. Adding a job, or widening what triggers one, is now a spend decision — treat it the way `ci.yml` already treats runner contention.
- **GitHub secret scanning and push protection are OFF** (they are free only on public repositories). The pre-commit hook runs `check-file-integrity.mjs` and does **not** scan for credentials, so `test:secret-hygiene` in CI is the backstop — do not weaken it.

**The automatic security scanner was CodeQL** (`codeql.yml`, added 2026-08), not the AI reviewer — and it is **DORMANT as of 2026-08-18**. Code scanning on a private repository requires a paid Code Security plan, so the analysis ran to completion, indexed all 2,399 files, and then had its upload rejected with *"Code scanning is not enabled for this repository"* — ~7 metered minutes per push for no finding anywhere, plus a red X on every PR. The automatic triggers are therefore off and only `workflow_dispatch` remains. **Nothing scans application logic automatically right now**; the `security-review` label is the escape hatch until this is restored. Four things to know before changing it:

- **Restoring it is two triggers plus a plan decision**, both spelled out in the header of `codeql.yml`. Do it once Code Security is enabled or the repository is public again — not before, or it goes straight back to failing.
...
- **Scope is guarded** by `test:codeql-scope`, because every way this breaks is silent — a source root added to `paths-ignore`, a `paths:` filter on the trigger, or a `config-file` pointing nowhere all yield a scan that reports "no new alerts" having examined nothing. The guard is dormancy-aware: it asserts the scan's *scope* in both states, prints a visible DORMANT warning while the triggers are off, refuses a half-restore (one automatic trigger back but not the other), and re-applies the full unfiltered-trigger rules the moment either returns.
