...
- **STOR-003 · Upload magic-byte verification — utility landed, adoption in progress** (2026-07-31). Every upload path validated only the client-declared `file.type`, which is forgeable (a script renamed `logo.png` passed an `image/png` allow-list; doc 06/18). New reusable verifier `src/utils/fileSignature.js` (`sniffMimeFromBytes` pure detection for PNG/JPEG/WebP/GIF/PDF/ZIP-OOXML + async `assertFileSignature(file, allowed)` that reads the real leading bytes and throws `file-signature-mismatch`; conservatively no-ops when it can't model any allowed type, so it never loosens the existing check). Adopted so far in the **picture bank** (`pictureBankService.assertUploadableImage`, both upload paths) and **teacher branding** (`uploadBrandingAsset`). Tests: `fileSignature.test.js` (`test:file-signature`), `pictureBankService.spec.js` "real bytes are not an image". **Remaining adopters (follow-up):** notes (`features/notes/lib/storage` inline image + doc), visual studio (`visualAssetService`), quiz/assessment component uploads (`LessonEditor`, `CreateQuizV2`, `EditQuizV2`, `AssessmentStudio`), admin syllabus/curriculum PDF panels, and the DOCX/scan import paths (`quizDocumentImport`, `importedQuizAssets`, `paperFigureAttach`). The `storage.rules` content-type + size caps remain the server-side backstop; a Storage-triggered server scan (covering all paths at once) is the heavier alternative noted in doc 06.
