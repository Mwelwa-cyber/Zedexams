...
- **P2 · `teacherLibraryService.getGeneration` still swallows errors** — the `useFirestore` half of this item is RESOLVED: `getAssessmentById` + the studio-edit hydration guard shipped in #2053, and `getMyAssessments` / `getAssessmentQuestions` now re-throw with per-surface error/retry UI (AssessmentList, both dashboards, TeacherLibrary, AssessmentResultsTab) in #2054. What remains: `teacherLibraryService.getGeneration` (`src/utils/teacherLibraryService.js:114`) still `catch`es every error and returns `null`, so a load failure on `LibraryItemDetail` renders as a false "deleted/not found" state with no retry. Fix is the same shape — re-throw + an error/retry state on `LibraryItemDetail`; own PR.
...
- ~~**P3 · Register dirty-state guard only covers the Back button**~~ — RESOLVED 2026-07-31. `MarkEntryGrid` now publishes its dirty flag to the Class Register shell via a small context (`registerUnsavedGuard.jsx`); `ClassRegisterDetail` guards the tab `<Link>`s, "← Class List", "Edit" and "Archive" with a confirm when marks are unsaved. (Declarative `<BrowserRouter>` has no `useBlocker`, and the register grid has no autosave, so the fix lifts state to the parent as noted.) Browser back/forward remains uncovered without a data router — the `beforeunload` handler still catches tab-close/reload. Tests: `registerUnsavedGuard.spec.jsx`.
