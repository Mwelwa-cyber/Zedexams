...
- ~~**P3 · Register dirty-state guard — in-register controls done, shell nav still open**~~ — RESOLVED 2026-07-31. #2063 shipped `registerUnsavedGuard.jsx` (dirty flag published by `MarkEntryGrid`; `ClassRegisterDetail` guards the tab `<Link>`s, "← Class List", "Edit", "Archive"); #2064 fixed the premature flag-clear (armed until the grid actually unmounts). Final piece (P1 from Codex review): the shared `TeacherLayout` nav (`dashboardV2/Sidebar.jsx`, mobile `NavDrawer`, bottom dock, account menu, logout) sits ABOVE the register's React context, so clicking Dashboard/Library there discarded unsaved marks silently. Fixed WITHOUT React context via a module-scope registry (`register/shellNavGuardCore.js`): the register publishes its confirm-gate (`setActiveShellNavGuard` in `useRegisterUnsavedGuard`), and `TeacherLayout` installs ONE capture-phase click listener that gates in-app anchor navigations **outside `<main>`** (so the register's own in-panel links, inside `<main>`, aren't double-prompted) via `confirmShellNavigation()` + the pure `isPlainInAppLinkClick` predicate (skips modified/new-tab/cross-origin clicks). The imperative button paths (logout, account-menu `navigate`) are gated at their chokepoints in `TeacherLayout`/`Sidebar`/`MobileDashboardView`. A no-op on every non-register teacher page (no guard registered → proceed). Browser back/forward still uncovered without a data router; `beforeunload` still catches tab-close/reload. Tests: `shellNavGuardCore.test.js` (`test:shell-nav-guard`), `registerUnsavedGuard.spec.jsx` "shell registry bridge", `TeacherLayout.spec.jsx` "shell-nav unsaved guard".
