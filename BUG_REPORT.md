...
- **STOR-003 · Upload magic-byte verification — utility landed, adoption in progress** (2026-07-31). Every upload path validated only the client-declared `file.type`, which is forgeable (a script renamed `logo.png` passed an `image/png` allow-list; doc 06/18). New reusable verifier `src/utils/fileSignature.js` (`sniffMimeFromBytes` pure detection for PNG/JPEG/WebP/GIF/PDF/ZIP-OOXML + async `assertFileSignature(file, allowed)` that reads the real leading bytes and throws `file-signature-mismatch`; conservatively no-ops when it can't model any allowed type, so it never loosens the existing check). The verifier also detects the OLE2 compound-file container so legacy office (`.doc`/`.xls`/`.ppt`) is validated rather than falsely rejected. **Adopted:** picture bank (`pictureBankService`, both paths), teacher branding (`uploadBrandingAsset`), notes (`features/notes/lib/storage` — `uploadInlineImage` + `uploadNoteFile`), and the admin **syllabus** + **curriculum** upload panels (PDF/DOCX/XLSX). `visualAssetService` is intentionally NOT wired — it uploads an app-generated canvas blob, not an untrusted user file. Tests: `fileSignature.test.js` (`test:file-signature`, incl. OLE + OOXML→zip mapping), `pictureBankService.spec.js` "real bytes are not an image". **Remaining adopters (follow-up):** quiz/assessment component uploads (`LessonEditor`, `CreateQuizV2`, `EditQuizV2`, `AssessmentStudio`) and the DOCX/scan import paths (`quizDocumentImport`, `importedQuizAssets`, `paperFigureAttach`). The `storage.rules` content-type + size caps remain the server-side backstop; a Storage-triggered server scan (covering all paths at once) is the heavier alternative noted in doc 06.
