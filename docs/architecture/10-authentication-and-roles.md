...
|---|---|---|---|
| AUTH-H1 | **High** | **Suspension not enforced at the rules layer.** `adminSetUserStatus` sets status + `revokeRefreshTokens`, but Firestore rules never check `status`, and the current ID token stays valid ~1h (rules don't check revocation). Client sign-out is a `window.alert`+`signOut` in the snapshot handler — a modified/ignoring client keeps full backend access up to an hour. | `adminUsers.js:52–69`, `firestore.rules:22–41`, `AuthContext.jsx:542–556` |
