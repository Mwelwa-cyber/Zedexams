...
|---|---|---|---|
| PAY-1 | **High (content confidentiality)** | **Learner premium *content* gating is client-side only.** Any verified user can read any `isPublished==true` quiz + its answer-bearing questions and published lessons directly from Firestore; the demo/paywall distinction lives only in React. Daily exams are the correctly server-gated exception. This is **not** a billing bypass (cannot flip the premium flag or steal paid AI generations) — it's a content leak. | `firestore.rules` quizzes 489–534, lessons 593–596; daily exams 511–532 |
