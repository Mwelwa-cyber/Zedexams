...
|---|---|---|---|
| PAY-1 | **High (content confidentiality)** — ✅ **RESOLVED (2026-07-17)** for quiz answer-keys + past-paper PDFs | **Learner premium *content* gating was client-side only.** Fixed: quiz `questions` read now requires `isDemo(parent) \|\| hasValidEntitlement()`; Storage `papers/` read now requires entitlement (or teacher/admin/owner). `hasValidEntitlement()` mirrors `hasPremiumAccess()` (read-time expiry). Lessons/notes were verified **not** premium-gated in the client (free content), so out of scope; making them premium is a scoped follow-up. Not a billing bypass. Tests: rules + storage emulator. See [`18`](./18-security-review.md), [`25`](./25-remediation-plan.md). | `firestore.rules`, `storage.rules` |
