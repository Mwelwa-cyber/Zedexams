...
- **BLOCKER — do not fix `AI-004` in isolation.** This is *"don't activate this until you've decided whether to keep it"*, **not** *"don't build this"*. The unmetered client-side path **already exists in shipped production code** (`src/utils/aiLogic.js` → `src/firebase/ai.js`, reached from two call sites); it is currently inert only because it is **failing closed on quota**. Fixing the quota therefore does not introduce the path — it makes an existing one operational, which is RISK-4 arriving in practice rather than in principle. Settle the routing question below *before, or together with,* the quota fix. (`AI-004` in [`BUG_REPORT.md`](../../BUG_REPORT.md).) The word is used in this repo's established sense, not as emphasis: [`16-production-blockers.md`](../production-readiness/16-production-blockers.md) defines a blocker as something that can reasonably cause, among other things, **"unrestricted expensive AI spend"** — which is exactly what making this path operational would enable, and is RISK-4's stated impact.
