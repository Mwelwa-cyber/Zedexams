...
- **BLOCKER: do not fix `AI-004`'s quota in isolation.** The unmetered client-side AI path already exists in production (`src/utils/aiLogic.js` → `src/firebase/ai.js`, two call sites); it is inert only because it fails closed on quota. Fixing the quota **activates** it — RISK-4 in practice rather than in principle. Settle the routing decision below before or with the quota fix. (`AI-004` in [`BUG_REPORT.md`](../../BUG_REPORT.md); "blocker" per [`16-production-blockers.md`](../production-readiness/16-production-blockers.md), which lists *"unrestricted expensive AI spend"*.)
- **Open question + measured scope (2026-08-05 console review — [`AUDIT_LOG.md`](../security/AUDIT_LOG.md)).** The open decision is whether these two features should route through the server-side Cloud Functions AI stack like the rest of the app, or keep calling AI Logic from the client. Three measurements that bear on it: the client AI Logic path has **exactly two consumers** — `src/features/classTimetable/services/timetableExtraction.js` (dynamic import) and `src/utils/forecastResourceSuggest.js` — so the migration surface is two call sites, not a sweep; `firebasevertexai.googleapis.com` served **one production request in six weeks, and it 429'd**, so nobody is currently being served by this path either way; and that 429 is the separate live bug called out in the blocker above.
