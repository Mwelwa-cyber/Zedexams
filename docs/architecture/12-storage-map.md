...
| STO-F2 | Low (P0 residual) | **`papers/` premium gate deployed 2026-07-17, but pre-existing download tokens may bypass it.** The `papers/` read now requires entitlement (SDK/`getDownloadURL()` path is gated), and no token URLs are persisted in Firestore. However, a Firebase download token is stable per object, so a tokenised past-paper URL minted/shared **before** the change stays valid until rotated. **Unverified** whether any were distributed; tokens were **not** rotated. *Follow-up:* audit + rotate premium-file metadata tokens, or move to authenticated/short-lived signed downloads. | SEC-F2 ([`18`](./18-security-review.md)), RISK-20 ([`23`](./23-risk-register.md)); `pastPapers.js` `getPaperPdfUrl` L280 |

> **P0 (2026-07-17):** the `papers/` read entitlement + `callerActive()` suspension gate (see the path register above) were merged (#1774) and **deployed to production** (Storage rules released). Storage rules govern **SDK / `getDownloadURL()` access**; they do not retroactively revoke already-issued tokenised URLs — see STO-F2.
