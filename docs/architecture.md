...
**Actual Phase 5 status, measured from the manifest: 195 of 196 export bodies have moved. One remains inline — `setUserRole`** (an auth trigger, classification `mechanical`, batch 1). The batch record §13 never carried: **batch 2** moved the 24 secrets-bound bodies (#2346), **batch 3** the 11 payment/webhook + audit-surface bodies (#2371), the latter under a since-lifted owner hold. What is genuinely still open from this phase is not the restructure but **the audit burn-down batch 3 explicitly deferred** — the unauthenticated HTTP surface and the uncapped AI callables — which its own commit message records as a separate, outstanding PR.
