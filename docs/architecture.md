...
- **Frozen for the duration, by owner instruction:** past papers, quizzes, games, and everything the Assessment Engine cutover touches, until the Phase 3 rollout flags reach 100%. That covers ~~`components/papers/`~~ (**released 2026-08-13 — see the rulings below**), `components/quiz/`, ~~`components/games/`~~ (**released 2026-08-14 — see the rulings below**), ~~`AssessmentStudio.jsx` + `teacher/studio/`~~ (**released 2026-08-13 — see the rulings below**) + `views/PaperBlocks|AssessmentPaperView`, `admin/PastPaperStudio`, `admin/CreateQuizV2`, and `DailyExamRunner`. Four of those are named on `scripts/visual/printAffectingPaths.js` by exact path, so they also carry the ~30-minute render gate — a second reason they go last rather than first.
  ...
  **What remains frozen, stated once so the strike-throughs above are not the only record:** `components/quiz/`, `views/PaperBlocks|AssessmentPaperView`, `admin/PastPaperStudio`, `admin/CreateQuizV2`, `admin/GamesSeedAdmin` and `DailyExamRunner`. All three migrations deliberately left every one of them where it was, and the games one verified it file-by-file with `git diff --quiet` rather than asserting it. Anyone reading this row for a surface not struck through should treat the freeze as fully in force, and should not read three releases in two days as the freeze having lapsed — the Phase 3 rollout is the condition, and it is tracked in §4, not here.
