...
- ~~Known hardening gaps (from `SECURITY_ENDPOINT_AUDIT.md`) to burn down during Phase 5: the unauthenticated HTTP surface and uncapped AI callables. Two current defaults deserve explicit review flags in code: moderation **fails open** on provider error, and rate limiting **fails open** on Firestore error.~~ **CLOSED.** The six audit items were burned down 2026-08-14 (see §4 of that file, and re-derive with the tests it names — `test:webhook-ledger`, `test:appcheck-http`, `test:ai-provider-inventory`, `test:library-download`, `test:http-request-guard`). The two fail-open defaults were **not** part of those six and outlasted them; closed 2026-08-16.

    **Rate limiting was already done** and is the pattern the other one was measured against: a structured `rate_limit_degraded` event plus `rateLimitHealth.js`, a canary that turns that telemetry into an actual ops alert.

    **Moderation was not, and the gap was worse than "no flag".** Its fail-open path logged a bare `console.warn` naming the provider error, and said nothing about the OUTCOME — a learner message passing UNSCREENED produced no record distinguishable from one that passed a clean screen. Two things made that unrecoverable rather than merely quiet: `functionErrorWatch`, the only thing watching server logs since #2230 (Sentry does not observe Cloud Functions), filters `severity>=ERROR`, so a WARNING is invisible to it rather than low-priority; and there is no moderation equivalent of the rate-limiter canary probing independently. A degraded child-safety screen on `apiAiChat` was therefore undetectable by any means the repo has.

    Now `contentModerationCore.describeModerationDegradation()` builds a structured `moderation_degraded` record and the shim emits it, at a severity that is a decision rather than a formatting choice: **fail-OPEN → `console.error`** (unscreened content reached a child-facing surface; ERROR is the floor for being seen at all), **fail-CLOSED → `console.warn`** (safety held, only availability suffered). A single provider blip does not page — `functionErrorWatch` alerts the non-OOM classes on a sustained rate across consecutive windows, per function, with a cooldown.

    **The fail-open default itself is unchanged**, deliberately: whether an unreachable provider should block a child's chat is a product decision with an availability cost, and `MODERATION_FAIL_CLOSED` already exists to flip it. What was missing was the ability to know it had happened. `test:moderation-core` (with a control proving the two failure modes cannot collapse into one severity) and `test:moderation` (which pins that the child-facing path actually emits, since a correct record nobody writes is the same bug).
...
- Public-read surfaces are deliberate and enumerated (`if true` reads): `examTimetables`, `announcements`, `leaderboards`, `daily_challenges`, `publicStats/global`, and token-shares readable by `get` only (never `list`) so they can't be enumerated.
- **`agentJobs` is admin-written, not server-only** — corrected 2026-08-05. Creation, review (`status`/`reviewedBy`/`reviewedAt`/`reviewNotes`/`overrideReason` only) and deletion are all `isAdmin()`; everything else on the document — `output`, `error`, `publishedRefs`, and every status transition beyond approve/reject — is server-owned. Create was `isTeacherOrAbove()` until the same date: that half of the grant was written for the teacher-facing agent-submission surface removed in 2026-06 and outlived it, so it was narrowed. The live client creator is the admin `BulkGenerateButton` — singular since `GenerateFromTopicMenu` was deleted as dead code (2026-08-14). Worth watching: a client-write grant justified by ONE component is one deletion away from being justified by none, at which point create should narrow to server-only rather than quietly outliving its reason, which is exactly how this grant came to be `isTeacherOrAbove()` for months after the surface it was written for was removed.
- **`settings` was counted among those and no longer is** — corrected 2026-08-16, closing the residual risk tracked in `docs/architecture/26-public-access-audit.md`. Its read is `isPublicSettingsDoc(settingId) || isAdmin()`, where the allowlist is exactly `global` (site name, maintenance flag, registration toggle — needed pre-auth) and `quizLibrary` (the public quiz runner's summary index). Every other id — `fxRate`, `curriculumUpdateSourceState`, and anything added later — is admin-read. The point is the DEFAULT: a settings doc created tomorrow is closed unless someone deliberately adds it to the allowlist, rather than public unless someone notices. Writes are admin-only, unchanged.
...
**Phase 3 — Assessment Engine. ALL THREE CUTOVERS BUILT; ALL THREE FLAGS STILL OFF.** Extract the engine (schema + normaliser + runner) seeded from `functions/shared/assessment/` and the best of `QuizRunnerV2`. Old runners keep working until their consumer is switched; results/leaderboard writes must stay byte-compatible. The plan is [`phase3-plan.md`](phase3-plan.md).

As of 2026-08-14 the games cutover (`timed_quiz`) has landed, which was the last item in that plan's §10.1 work order. **This is not the same thing as the phase being finished**, and the distinction is the one the freeze clause below turns on: the code for all three runners now exists behind `featureFlags.assessmentEngine.{pastPaperQuiz,quiz,game}`, and every one of those switches — plus `rolloutPercent` — is still at its fail-closed default. No learner has met the engine. What remains is the ramp, which is an operator sequence rather than a pull request: rehearse the rollback on the live toggle (§5.1 criterion 7, the one criterion no diff can discharge), then flip and raise past-paper → quizzes → games, watching §7.2's `assessment_engine_path` event and §7.3's rollback triggers. Do not read "the cutovers are built" as the rollout condition being met.
...
- **~~Frozen for the duration, by owner instruction~~ — CLOSED 2026-08-14 by the sixth owner ruling; see the paragraph beginning "THE FREEZE IS CLOSED" below.** The clause read: past papers, quizzes, games, and everything the Assessment Engine cutover touches, until the Phase 3 rollout flags reach 100%. That covered ~~`components/papers/`~~ (**released 2026-08-13 — see the rulings below**), ~~`components/quiz/`~~ (**released 2026-08-14 — see the rulings below**), ~~`components/games/`~~ (**released 2026-08-14 — see the rulings below**), ~~`AssessmentStudio.jsx` + `teacher/studio/`~~ (**released 2026-08-13 — see the rulings below**) + ~~`views/PaperBlocks|AssessmentPaperView`~~ (**moot — already inside `assessmentStudio`**), ~~`admin/PastPaperStudio`~~ (**released 2026-08-14**), ~~`admin/CreateQuizV2`~~ (**released 2026-08-14 — see the rulings below**), and ~~`DailyExamRunner`~~ (**released 2026-08-14**). Four of those were named on `scripts/visual/printAffectingPaths.js` by exact path, so they also carry the ~30-minute render gate — a second reason they go last rather than first.
  ...
  **A THIRD owner ruling landed on 2026-08-14, releasing `components/games/`** — migrated the same day, and recorded in the Wave 4 entry below. Like session E's, it was granted against an **unmet** condition rather than a misdescribed boundary: the rollout flags were re-measured, found still at their fail-closed defaults, and the ruling asked for anyway. It is a decision about one surface, not a re-reading of this clause. The freeze itself has not lifted and its condition is unchanged.

  **A FOURTH owner ruling landed on 2026-08-14, releasing `components/quiz/`** — the same day as the games one, and migrated the same day. Same shape as the other three: the rollout flags were re-measured, still at their fail-closed defaults, and the ruling asked for anyway.

  **It released the DIRECTORY, not everything the directory serves, and that distinction did the work.** `admin/CreateQuizV2` is its own freeze-list entry and was not released; it imports five modules from `components/quiz/`, whose transitive closure inside that directory is **29 modules** — effectively the whole import/parse/review body. So the migration moved the learner-facing RUNTIME (`QuizRunnerV2`, `QuizResultsV2`, `QuizList`, `QuizTip`) into `features/quiz`, promoted the eight components three areas share into `src/shared/components/`, and left the AUTHORING surface in place. That was measured before anything moved: the runner trio's closure is 12 modules and shares exactly TWO with the editor — `ExtraQuestionImages` and `ZoomableImage` — both of which already qualified for `shared/` on consumer count. Had the overlap been larger the split would not have been a migration, and the answer would have been to report that rather than to shim thirty modules.

  **A FIFTH owner ruling landed on 2026-08-14, releasing `admin/CreateQuizV2`** — hours after the fourth, and for a reason the fourth had recorded: releasing `components/quiz/` without it left 38 modules of quiz AUTHORING pinned by one admin page. Measured on the branch, `CreateQuizV2` is not a *consumer* of that code, it is a PAGE of it — the admin's create-a-quiz screen beside `EditQuizV2`'s edit-a-quiz screen, rendering the same three editor components. Leaving it in `components/admin/` would have split one feature across two homes, which is the mistake the `adminImages` entry above was explicitly not allowed to make.

  **THE FREEZE IS CLOSED. A SIXTH OWNER RULING, 2026-08-14, RELEASED THE WHOLE REMAINING LIST**, and unlike the five before it this one is not a carve-out: it ends the clause rather than naming one surface. Everything below this paragraph describing what "remains frozen" is now HISTORY — kept because the strike-throughs are not a sufficient record on their own, and because the reasoning is what a future freeze should be argued against, not because any of it is in force.

  Read the condition honestly: **the Phase 3 rollout flags are still at their fail-closed defaults**, exactly as they were for rulings three, four and five. This ruling did not meet the stated condition; it replaced it. The engine cutovers are still built-not-rolled-out, no learner has met the engine, and the ramp described in §4 is unchanged and still outstanding. What the ruling decided is that the Phase 4 MOVE no longer waits on the Phase 3 RAMP — which is the distinction the clause itself drew all along ("the test is whether the file moved").

  All five remaining surfaces migrated the same day: `admin/GamesSeedAdmin` → `features/games/pages/` (with `utils/gamesService.js` → `features/games/services/`, paying the §14.2 debt that front door had recorded); `DailyExamRunner` + the whole `components/exams/` surface → `features/dailyExams/`, retiring the six shims on the condition they wrote for themselves; `admin/PastPaperStudio` + `admin/AdminPastPapers` → `features/adminPastPapers/`, which emptied `src/components/admin/`; and that in turn freed the quiz document-import body to `src/services/quizImport/`, which emptied `src/components/quiz/`. `views/PaperBlocks|AssessmentPaperView` moved last, and that entry is worth reading closely, because the first pass through this list got it WRONG. It was recorded here as "needed nothing — already travelled inside `assessmentStudio`", on the strength of `features/assessmentStudio/components/views/` existing. It does exist, and it holds `PaperPagesPreview` and `TableOfSpecificationView` — not these two, which were still sitting in `src/components/teacher/views/`. The claim was checked against a directory NAME rather than against the files, which is the same class of error as classifying a util by a path substring. `src/curriculum/diagrams/index.js` is what caught it: that front door names `components/teacher/views/PaperBlocks` as one of two frozen readers it had to work around, which cannot be true of a file that had migrated.

  Corrected and done: `PaperBlocks` (933 lines + its 303-line spec) went to a NEW engine, `src/engines/paper-render/`, and `AssessmentPaperView` to `features/teacherLibrary/components/` on the sole-consumer rule. The engine is forced rather than chosen — three features read `PaperBlocks`, so it must sit below all of them, and from there `shared` is refused it (it imports `curriculum/diagrams`), `services` is the wrong meaning, `curriculum` is the wrong classification, and `export-engine`'s index is an explicit contract about modules that produce a FILE, which a JSX renderer does not. `assessmentStudio.css` came down to `src/shared/styles/` with it, for the same shape of reason one layer further down.

  **Three legacy directories are gone as a result** — `components/admin/`, `components/exams/`, `components/quiz/` — and the debt lists did not grow: `test:import-boundaries` records 3 cross-feature and 3 legacy imports before and after. The one recorded legacy entry that remains, the `UpgradeModal` shim, is NOT retired by this ruling: its own retirement condition is the rollout reaching 100%, which is a different condition and is still unmet.

  *(Historical, from before the sixth ruling:)* **What remains frozen, stated once so the strike-throughs above are not the only record:** `views/PaperBlocks|AssessmentPaperView`, `admin/PastPaperStudio`, `admin/AdminPastPapers`, `admin/GamesSeedAdmin` and `DailyExamRunner` (with its spec). All four migrations deliberately left every one of them where it was, and the last two verified it file-by-file with `git diff --quiet` rather than asserting it. Anyone reading this row for a surface not struck through should treat the freeze as fully in force, and should not read four releases in two days as the freeze having lapsed — the Phase 3 rollout is the condition, and it is tracked in §4, not here. **Note that three of the six now hold code back by DEPENDENCY rather than by their own content:** `CreateQuizV2` pins 38 modules of quiz authoring, `DailyExamRunner` pins nothing but is reached by six shims, and `GamesSeedAdmin` pins `utils/gamesService.js`.

  **`admin/GamesSeedAdmin` is now named here, and was always covered.** It was reached by the dependency test rather than by path — it writes the `games` collection through `utils/gamesService.upsertGame` — which is the same signal that put `AdminCsvImport` and `AdminDashboard` inside the freeze. Naming it explicitly costs nothing and stops the next reader having to re-derive it; it is also the single reason `utils/gamesService.js` could not travel into `features/games/`.

  **What this clause freezes is the Phase 4 MOVE, not the Phase 3 cutover.** A cutover edits the runner in place to add its flag branch, which is what the freeze exists to protect — the file stays where every other consumer expects it while both paths are live. So `QuizRunnerV2.jsx` was edited under this clause for the quizzes cutover and `TimedQuizGame.jsx` was edited under it for games, and neither was a release: `components/quiz/` and `components/games/` were exactly as frozen after those diffs as before them. The test is whether the file moved. (`TimedQuizGame.jsx` has since moved, under the 2026-08-14 ruling above and one day after that cutover landed — which is what a release looks like, as distinct from what an edit looks like.)
...
- **Wave 4, the larger teacher areas:** ~~`scan`~~ (**never a migration of its own — resolved 2026-08-13; see the note below beginning "Resolved 2026-08-13"**), `classList`, `schemeOfWork` + `weeklyForecast` (paired, because `schemeFormat.js` is shared between them and moves to `src/shared/utils/` on the second), `sba`, `classTimetable`, `markSchedule`/`recordOfWork`, `teacherClasses`, `curriculumBrowsers`, `teacherLibrary` (blocked on the `engines/export-engine` decision — 40 of its imports are exporters), `register`, `dashboardV2`, then the remaining admin areas.
...
**What is left in `src/components/admin/`, and why each stays.** Eight files, and none of them is a judgement call: `PastPaperStudio.jsx` (+ spec), `CreateQuizV2.jsx`, `AdminPastPapers.jsx` (+ spec), `GamesSeedAdmin.jsx` and `pastPaperReport.jsx` (+ spec, whose only consumer is `PastPaperStudio`) are named on the freeze list by path and were **not** part of this ruling. ~~`GenerateFromTopicMenu.jsx` is **dead code**~~ — **deleted 2026-08-14**, as its own PR, which is what that entry said the fix should be. The reasoning it recorded stands and is why it was not swept into `adminCbcKb`: filing dead code into a new feature gives it a home it has not earned. **The row is closed again on the remaining eight.**
...
**`subscription` — CLAIMED 2026-08-14 by session A**, branch `claude/subscription`, on an owner instruction to complete Phase 4. Forty-seven files, ~4,200 lines of source: `components/subscription/` plus four sole-consumer utils (`pendingPremiumAction`, `lockedFeature`, `reminderVisibility`, `usageReminderLogic`, each with its spec). The last and most widely consumed area of the wave.

**The three frozen consumers are reached by a SHIM, not by an edit — and the freeze holds.** `components/quiz/QuizRunnerV2`, `components/quiz/QuizList` and `components/games/PlayGame` import `UpgradeModal`, as do three `QuizRunnerV2` specs via `vi.mock`. All six are **byte-identical to `main`**, verified with `git diff --quiet` per file rather than asserted. `components/subscription/UpgradeModal.jsx` survives as a one-line re-export shim carrying its own retirement condition.

**This supersedes the earlier recommendation to defer, and the reason is worth recording.** The advice against a blanket "import edits in frozen files are permitted" stands — that precedent gets cited later for edits that are not harmless. What was under-weighted is that the shim makes the question moot: the migration goes AROUND the frozen files rather than through them, so no exception is granted and nothing frozen changes. The pattern is the repo's own (`src/utils/unresolvedFigures.js` and four neighbours onto `functions/shared/assessment`, policed by `test:shim-guard`).

**The shim points at the COMPONENT, not the front door, and that is deliberate.** Routing it through `index.js` would resolve cleanly and need no debt entry — but it would then evaluate every module that index re-exports, putting the status banner, `PremiumGate` and `RenewalBanner` into the chunk of the highest-traffic learner route mid-cutover. Trading one recorded, retirement-dated debt entry for unmeasured weight on that route is the wrong way round. Legacy imports 2 → 3, forced and documented, on the precedent that list already sets.

**Measured, not argued.** The `QuizRunnerV2` chunk went 55,945 → 55,980 bytes (**+35**, one module hop) and contains zero `SubscriptionStatusBanner` and zero `AttemptCounter` strings — the shim delivers exactly `UpgradeModal`, as before. The main bundle went 163,406 → **156,636** (−6,770), because the six paywall mounts now resolve to feature paths and chunk boundaries settled better than they had.
...
**What this correction does NOT claim.** `src/utils/` still holds ~263 non-test modules and remains the catch-all, and the Phase 3 rollout flags are still at their fail-closed defaults. Those are separate open items, not consequences of this one.

**And a correction to the correction, same day.** The first version of this paragraph said *"Phase 5 has not started (47 handlers still inline, 12 of §12's domain directories absent)"*. **Both halves were wrong**, and the way they were wrong is worth keeping because the same mistake is available to the next reader:

- The "47 inline handlers" was a count of `on*()` declarations in `index.js` **including their frozen options blocks** — `region`, `secrets`, `timeoutSeconds`, `memory`, `enforceAppCheck`. Those blocks are *required* to stay in `index.js`: it is where `test:functions-manifest`'s frozen-surface guard reads them from, exactly as batch 3 recorded ("the builders and every frozen option stay in index.js where the guard reads them"). A handler whose body has moved still presents ~10 lines there, so a line-count heuristic reads a completed migration as an untouched one. 45 of the 47 are thin delegates. The authoritative measure is the `inline` flag in `scripts/functions-manifest.json`, not the shape of `index.js`.
- The "12 of §12's domain directories absent" compared the tree against §12's *aspirational* directory names (`ai/`, `assessments/`, `payments/`, `middleware/`, …). The batches never adopted that scheme — they moved bodies into `paymentHandlers.js`, `httpSurfaceHandlers.js` and the ~20 domain directories that already exist. Judging the phase by §12's names measures adherence to a naming proposal, not whether the work happened.

**Actual Phase 5 status, measured from the manifest: 195 of 196 export bodies have moved. One remains inline — `setUserRole`** (an auth trigger, classification `mechanical`, batch 1). The batch record §13 never carried: **batch 2** moved the 24 secrets-bound bodies (#2346), **batch 3** the 11 payment/webhook + audit-surface bodies (#2371), the latter under a since-lifted owner hold. What batch 3's commit recorded as still open — **the audit burn-down**, the unauthenticated HTTP surface and the uncapped AI callables — was in fact closed the SAME DAY, hours after that message was written (`SECURITY_ENDPOINT_AUDIT.md` §4: all six items, 2026-08-14, one PR each; item 3 turned out to have been done already). Verified 2026-08-16 by running the five suites those entries name rather than by reading them. **A commit message is a snapshot at the moment of writing and does not learn** — this is the third status claim in this document's orbit found stale in one session, after Phase 4's "complete" and Phase 5's missing batch record.

The one item that genuinely did outlast the six was the pair of fail-open defaults in §7 — moderation and rate limiting. Rate limiting was already handled; moderation was not, and is now (see §7). Phase 5's remaining work is therefore `setUserRole` alone.
...
**Wave 4 — `games`, the second surface released from the owner freeze** (2026-08-14). `/games` and everything under it: the hub, the subject selector, the game list, the play surface and the global leaderboard — 45 files, ~9,700 lines, five route mounts. Five screens into `pages/`, twenty-four components into `components/` (the eight game engines among them, plus both `TimedQuizGame` specs), `useGameFinish` into `hooks/`, and eleven pure modules into `lib/` — the seven `*Core` files, `timedQuizRound`, `gameFeel`, `subjectMascots`, and `gameSounds` from `src/utils/`. Two exported names: `GameBadgeCard` and `getSubjectMascot`.

**The freeze had NOT expired — this moved on an explicit owner ruling, 2026-08-14**, the third such ruling and the second granted against an unmet condition rather than a misdescribed boundary (session E's `components/papers/` was the first). Measured before acting, exactly as that one was: every `featureFlags.assessmentEngine.*` switch still defaults off and `rolloutPercent` still defaults to 0. **The freeze clause is edited in place to say so**, because a reader who finds the clause and not this paragraph gets the wrong answer. **What remains frozen is unchanged and was verified file-by-file with `git diff --quiet`, not asserted:** `components/quiz/`, `views/PaperBlocks`, `views/AssessmentPaperView`, `admin/PastPaperStudio`, `admin/CreateQuizV2`, `admin/GamesSeedAdmin` and `DailyExamRunner` are byte-identical to `main` after this PR.

**Games is the engine's LAST cutover, and nothing about it moved.** `TimedQuizGame.jsx` reads `useAssessmentEngineFlag('game')` and still carries both paths; `TimedQuizGame.engine.spec.jsx` and `TimedQuizGame.legacy.spec.jsx` moved beside it and differ only in their mock paths, so the rollback the flag selects is still tested from both sides.

**Three consumers were invisible to `git grep`, and the resolver found all three — the same blind spot `papers` hit, one wave later.** Searching `components/games` returns the five route mounts and the two learner-dashboard imports and stops; it cannot match `import Confetti from '../games/Confetti'`, because a sibling-relative specifier never contains the string being searched for. `test:import-boundaries` reported `components/exams/DailyExamsHub`, `components/exams/ExamCelebrations` and **`components/quiz/QuizList`** — the last of which is frozen, which is how a one-line edit inside the freeze was found before it was made rather than after.

**`GameStickerStyles` and `Confetti` went DOWN to `src/shared/components/`, and the bundle is the argument.** The first draft kept both in the feature and exported them from the front door. Measured, that cost **+75 kB of static edges each on `GradeHub`, `SubjectDrillDown` and `ExamResultsPage`** — three learner routes that render no game — because Rollup groups a front door's re-exports into one chunk, and another name on that door (`GameBadgeCard`) reaches `utils/gamesService` and the Firebase client. Both pass §14.6 on reach (`GameStickerStyles` imports **nothing**; `Confetti` imports React) and both are drawn outside games, so they belong below both callers on the `studioFields` / PDF-viewer precedent. Re-measured after: **+124 bytes** across the whole bundle and no route gains a chunk. This is MIGRATION_TEMPLATE.md §4's warning arriving for the second time and the first time it was *caught by measurement rather than by a spec failing* — tree-shaking answers "how many bytes land in this chunk"; a front door's real cost is "what does importing one name make the browser fetch".

**The frozen consumer is reached by a SHIM, and this one costs no debt entry.** `components/games/GameStickerStyles.jsx` survives as a one-line re-export, so `QuizList.jsx` is byte-identical and the migration goes AROUND the frozen file rather than through it — #2342's pattern. Because the shim points at `src/shared/components/` rather than into a feature, the boundary scan has nothing to record: the legacy tree may read the shared layer. **`KNOWN_LEGACY_FEATURE_IMPORTS` is unchanged at 3 and `KNOWN_CROSS_FEATURE_IMPORTS` at 3** — a games migration that ended with either list one line longer would have been the signal §8 describes, and moving the component below both callers is what removed the line rather than recording it. The intermediate state where the entry WAS required is kept in that file's comments, because the cheaper shim is only visible once you know the expensive one was on the table.

**The `src/utils/` slice mostly could not travel, and one module was moved and moved back.** `gamesService.js` is read by `admin/GamesSeedAdmin.jsx`, which is named on the freeze list by path, and by `hooks/useLearnerSearch`; `gameBadgesService.js` by `learnerDashboard/BadgesPage`; `gameProgress.js` and `gameScorePayload.js` by `gamesService` itself. `gamesIntelligence.js` was migrated into the feature and reverted within the same change: `gamesService.js` reaches it through `await import('./gamesIntelligence')`, a **dynamic** import no lint rule inspects, so the move would have converted a legal downward import into a new debt line. Only `gameSounds.js` had games as its whole consumer set, and only it travelled. **§14.2 is knowingly not satisfied** and the feature has no `services/` directory — its Firebase access is `utils/gamesService.js`, which cannot move while `GamesSeedAdmin` is frozen, and an empty `services/` would suggest a boundary that does not exist. Recorded in the feature index, as `adminCbcKb` recorded its own.

**A path-classified guard moved with the code, and it would have passed by covering nothing.** `gamesDarkSurface.test.js` read its own directory flat for `.jsx` files; against a feature laid out in `pages/`/`components/`/`lib/` that read returns **zero**, and the neutral-vocabulary scan then finds no violation because it examines no file. It now walks recursively, and the three files that render inside `.force-light-theme` from OUTSIDE the feature — `components/quiz/QuizList.jsx` and the two promoted shared components — are named by path with an `existsSync` assertion, so a later move reports rather than silently narrows the guard. The `>= 20` floor is what turns the empty read into a failure and is kept next to the walk for that reason.

**Twelve node test scripts point at the moved `lib/` modules by literal path** (the nine game cores, the round machine, `gameFeel`, and `test-exam-calendar-fix`'s mascot import) and were re-pointed in the same commit, alongside `test:games-dark-surface`'s own path in `package.json`. `run-all-tests.mjs` discovers by npm-script name, none of which changed, so the count is unchanged at **718** — a script whose path stops resolving does not report less, it throws, but only if something still runs it.

**Verification.** `eslint .` at **0 errors** and the same 4 pre-existing warnings as `main` — the shim adds none, because it points at `shared/`. `test:import-boundaries` 3 cross-feature / 3 legacy, `test:mock-paths` 1,001 paths, `test:all` 718 node scripts, `test:unit` 370 files / 3,860 tests, `npm run build` clean at 578 chunks against 577. No rule, index or collection changed, so §14.12's emulator obligation does not attach.

**Wave 4 — `quiz`, the learner-facing runtime, and the first release that could NOT take its whole directory** (2026-08-14). `/quizzes`, `/quiz/:quizId` and `/results/:resultId`: `QuizRunnerV2` (2,248 lines) with all three of its specs, `QuizResultsV2` + spec, `QuizList`, and `QuizTip` — 9 files into `features/quiz/`. **Empty front door**, on the `blog` / `uiAudit` / `adminSettings` precedent: all three screens are route-mounted lazily and nothing else in `src/` imports them.

**Released by owner ruling the same day as games, and the interesting part is what the ruling could not reach.** `admin/CreateQuizV2` is its own freeze-list entry and stays frozen. It imports five modules from `components/quiz/`, and the transitive closure of those inside the directory is **29 modules** — the whole import/parse/review body. So the directory splits: the RUNTIME migrates, the AUTHORING surface (38 modules) stays and becomes `features/quizEditor` when `CreateQuizV2` is released. This is the first Phase 4 item where "migrate X" and "empty X's directory" are different requests, and the difference was measured rather than discovered halfway through.

**The seam was measured BEFORE anything moved, and it is what made the split legitimate.** The runner trio's closure is 12 modules; the editor cluster's is 29; the intersection is exactly **two** — `ExtraQuestionImages` and `ZoomableImage`. Both already qualified for `src/shared/components/` on their own consumer count, so promoting them removed the only overlap at no cost. Had the intersection been thirty, the honest report would have been that the runner is not separable, not thirty shims.

**Eight components went DOWN to `src/shared/components/`, each drawn from three different areas.** The reading-assist cluster (`PassageViewer`, `ReadingSettingsButton`, `ReadingSettingsSheet`, `TextToSpeechButton`, `ThemePreview`) and `QuizReviewScreen`, plus the two above — read by `features/quiz`'s runner, `features/papers/PublicQuizRunner`, and `components/exams/DailyExamRunner`. §14.6 checked on reach: six import nothing but React, `ThemePreview` nothing at all, `PassageViewer` reaches `editor/RichContent` and no Firebase.

**Six shims, and the reason they cost nothing is the finding worth carrying forward.** `DailyExamRunner` is on the freeze list in its own right — outside Phase 3 entirely, retiring with the Daily Quiz rework — and was not covered by this ruling, so it reaches six of the promoted components through one-line re-exports at the old paths and is byte-identical to `main` (spec included). Because those point at `shared/` rather than into a feature, **`KNOWN_LEGACY_FEATURE_IMPORTS` stays at 3 and `KNOWN_CROSS_FEATURE_IMPORTS` at 3**. Seven shims were created across the two migrations of 2026-08-14 and not one is on either list: when the module a frozen file needs can honestly live BELOW both callers, promoting it is simultaneously the cheaper shim and the better placement. That is now recorded in the guard itself rather than in a PR description.

**One shim was created and retired within the same day, by the event its own docblock predicted.** The games migration left `components/games/GameStickerStyles.jsx` for the then-frozen `QuizList`. This migration moved `QuizList`, so it imports the shared component directly, the shim lost its only consumer, and **`src/components/games/` is now empty and removed.** The argument for writing retirement conditions down rather than trusting a future reader to re-derive them has rarely been demonstrated this fast.

**`components/subscription/UpgradeModal.jsx` now has NO frozen consumer**, which changes its retirement condition rather than discharging it. All six references are in `features/quiz` and `features/games`. What still holds the shim is #2342's chunk measurement, which never depended on the freeze — routing them at `features/subscription` puts the status banner, `PremiumGate` and `RenewalBanner` into three high-traffic learner chunks. Its retirement is now a measurement, not a date, and deliberately not smuggled into a file move.

**The migration waited for #2372 rather than racing it.** That PR was open and editing `QuizRunnerV2.jsx` + `QuizRunnerV2.spec.jsx` while this was being planned; both files are renamed here. A rename landing across an open edit is the one collision git resolves textually while silently dropping a side, and the side at risk was a Phase 3 behaviour decision with its two new regression cases. It merged at 09:36; this started after. **The `.characterisation.spec.jsx` baseline travelled intact** — it is the recorded byte-compatibility reference the cutover is measured against, so it is load-bearing rather than incidental.

**Verification.** `eslint .` 0 errors and the same 4 pre-existing warnings; `test:import-boundaries` 2,128 files, 3/3, none new; `test:mock-paths` 1,001; `test:all` **718** node scripts (unchanged — all eleven quiz `test:*` scripts point at modules that stayed); `test:unit` 370 files / 3,866 tests; `check:bundle-edges`; `check:integrity`. Every relocated file diffed against its blob at `main`: **only import and `vi.mock` lines differ, in all fifteen.** The bundle is the cleanest of the phase — **578 chunks before and after, +1 byte total**, and every route closure measured (`QuizRunnerV2`, `QuizResultsV2`, `QuizList`, `DailyExamRunner`, `PublicQuizRunner`, `EditQuizV2`, `CreateQuizV2`, `GradeHub`) unchanged at ±0.

**Wave 4 — `quizEditor`, the other half of the quiz split** (2026-08-14). The authoring surface: `EditQuizV2` (2,680 lines) + spec, **`CreateQuizV2` (1,752) out of `components/admin/`**, 20 editor components with five specs, and seven pure modules into `lib/` — 44 files. Two exported names, `ImageCropModal` and `QuizVerifyModal`, both for `features/assessmentStudio`. Both pages stay route-mounted lazily.

**`CreateQuizV2` is a PAGE of this feature, not a consumer of it**, which is why the fifth ruling was worth asking for rather than shimming around. It renders `QuizSectionsEditor`, `QuizEditorPreviewPanel` and `QuizValidationChecklist`, and sits beside `EditQuizV2` as create-a-quiz beside edit-a-quiz. Five shims would have kept one feature in two homes.

**`ImportReviewBadge` was on the front door for about ten minutes and is not.** Its only consumer anywhere is `features/adminContent/ManageContent`, so by the sole-consumer rule it is that feature's component and it moved there instead. That discharges a debt `adminContent`'s own index recorded — it noted `ManageContent` "still imports `ImportReviewBadge` … across the legacy boundary", which is no longer true. A name reaching a front door because a migration put it there, rather than because anything asked, is the cheapest kind of over-export to catch.

**The document-IMPORT body stays in `src/components/quiz/`, and it is a constraint rather than a deferral.** `documentQuizImporter` (1,524 lines) plus `documentQuizParserCore`, `documentQuizTableBlocks`, `documentQuizParagraphRuns`, `documentQuizReconcile`, `scannedQuizImporter`, `importFormatChecks` and `pastPaperParts`. `src/utils/paperToQuizConverter.js` imports it and is itself pinned by the still-frozen `admin/AdminPastPapers`, so moving it here would be a legacy→feature debt entry. **And it cannot go DOWN either**, the way this migration's other forced moves did: measured, its closure is 55 modules and reaches **eight** Firebase entry points through `utils/aiAssistant`, and `shared` is in `NO_FIREBASE_LAYERS`. Its own dependencies stay with it, because a legacy module importing feature internals is the same debt pointing the other way.

**`importRichText` + `importFormatTokens` went to `src/shared/utils/`, on the sharpest forced move in the phase.** `src/utils/toolNotationRender.js` reads `importRichText`, and `src/engines/export-engine/homeworkToPdf.js` and `worksheetToPdf.js` read *that* — and **an engine may not import from a feature** (§12). Same rule that sent `weeklyForecast.js` and `sbaTaskToPaper.js` down, and unlike `documentQuizImporter` the reach permits it: closure of two modules, zero packages, no DOM, no Firebase.

**Two utils came IN.** `src/utils/paperPageProvider.js` and `src/utils/quizReimportDiff.js` (+ node test) had exactly one consumer each — `EditQuizV2` — so moving the caller alone would have converted two legal legacy imports into two debt entries. `paperFigureAttach.js` did NOT: the frozen `admin/PastPaperStudio` reads it too.

**Four `vi.mock` paths were the migration's real trap, and only one of the two guards could see it.** After re-pointing `assessmentStudio` and `adminContent` at the front door, their four mocks still named `quizEditor/components/…` — paths that RESOLVE, so `test:mock-paths` stayed green while every one of them was a silent no-op with the specs exercising the real canvas-heavy crop modal and the real verify modal. Caught by reading the diff, not by a guard. This is MIGRATION_TEMPLATE.md step 0's first table row, and the case it warns about specifically: the mock that gets missed is the CONSUMER's, not the moved file's.

**Five path-literal references outside `src/`** — `test-import-rich-text-roundtrip.mjs` (which failed loudly), `test-option-letters.mjs`, and three comments in `functions/` — were re-pointed. The rewrite script walked `src/` only; the round-trip test is what said so.

**Verification.** `eslint .` 0 errors and the same 4 pre-existing warnings; `test:import-boundaries` 3 cross-feature / 3 legacy, none new; `test:mock-paths` 1,001; `test:all` **720**; `test:unit` 370 files / 3,866 tests. Every still-frozen file verified byte-identical to `main` with `git diff --quiet`.
