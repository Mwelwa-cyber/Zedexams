...
- **Frozen for the duration, by owner instruction:** past papers, quizzes, games, and everything the Assessment Engine cutover touches, until the Phase 3 rollout flags reach 100%. That covers ~~`components/papers/`~~ (**released 2026-08-13 — see the rulings below**), `components/quiz/`, `components/games/`, ~~`AssessmentStudio.jsx` + `teacher/studio/`~~ (**released 2026-08-13 — see the rulings below**) + `views/PaperBlocks|AssessmentPaperView`, `admin/PastPaperStudio`, `admin/CreateQuizV2`, and `DailyExamRunner`. Four of those are named on `scripts/visual/printAffectingPaths.js` by exact path, so they also carry the ~30-minute render gate — a second reason they go last rather than first.
  ...
  **Two owner rulings, both 2026-08-13, released two different halves of this clause on the same day** — the Assessment Paper Studio (session F) and the past-paper archive (session E). Both migrated that day (see the two Wave 4 entries below), and neither ruling touched the other's surface. The studio ruling was made against the measured inventory, not the path list — which is the third time this phase the boundary has been settled by what the code reaches rather than by what the clause names, and the second time in that direction. **The rest of the clause stands unchanged**: `components/quiz/`, `components/games/`, `views/PaperBlocks|AssessmentPaperView`, `admin/PastPaperStudio`, `admin/CreateQuizV2` and `DailyExamRunner` remain frozen until the rollout flags reach 100% — **they are still at 0** — and both migrations deliberately left every one of them where it was. Anyone reading this row for a *different* surface should treat the freeze as fully in force.
...
| **`papers`** — CLAIMED 2026-08-13 by session E, branch `claude/papers-migration-v0u62a`; **released from the freeze by owner ruling the same day**, see the freeze row above and the Wave 4 entry below | — | — |
