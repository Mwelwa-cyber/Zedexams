...
- **Frozen for the duration, by owner instruction:** past papers, quizzes, games, and everything the Assessment Engine cutover touches, until the Phase 3 rollout flags reach 100%. That covers `components/papers/`, `components/quiz/`, `components/games/`, ~~`AssessmentStudio.jsx` + `teacher/studio/`~~ (**released 2026-08-13 — see the ruling below**) + `views/PaperBlocks|AssessmentPaperView`, `admin/PastPaperStudio`, `admin/CreateQuizV2`, and `DailyExamRunner`. Four of those are named on `scripts/visual/printAffectingPaths.js` by exact path, so they also carry the ~30-minute render gate — a second reason they go last rather than first.

  **Owner ruling, 2026-08-13 (session F): the freeze is LIFTED for the Assessment Paper Studio**, and it migrated the same day (see the Wave 4 entry below). The ruling was made against the measured inventory, not the path list — which is the third time this phase the boundary has been settled by what the code reaches rather than by what the clause names, and the second time in that direction. **The rest of the clause stands unchanged**: `components/papers/`, `components/quiz/`, `components/games/`, `views/PaperBlocks|AssessmentPaperView`, `admin/PastPaperStudio`, `admin/CreateQuizV2` and `DailyExamRunner` remain frozen until the rollout flags reach 100%, and the migration deliberately left every one of them where it was. Anyone reading this row for a *different* surface should treat the freeze as fully in force.
...
| `StudioHeader.jsx` + `studioHeader.css` + spec | `lessonPlanStudio` only | → into that feature |
