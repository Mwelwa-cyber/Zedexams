...
- **Frozen for the duration, by owner instruction:** past papers, quizzes, games, and everything the Assessment Engine cutover touches, until the Phase 3 rollout flags reach 100%. That covers ~~`components/papers/`~~ (**released 2026-08-13 — see the rulings below**), ~~`components/quiz/`~~ (**released 2026-08-14 — see the rulings below**), ~~`components/games/`~~ (**released 2026-08-14 — see the rulings below**), ~~`AssessmentStudio.jsx` + `teacher/studio/`~~ (**released 2026-08-13 — see the rulings below**) + `views/PaperBlocks|AssessmentPaperView`, `admin/PastPaperStudio`, `admin/CreateQuizV2`, and `DailyExamRunner`. Four of those are named on `scripts/visual/printAffectingPaths.js` by exact path, so they also carry the ~30-minute render gate — a second reason they go last rather than first.
