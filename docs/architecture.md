...
- **Frozen for the duration, by owner instruction:** past papers, quizzes, games, and everything the Assessment Engine cutover touches, until the Phase 3 rollout flags reach 100%. That covers ~~`components/papers/`~~ (**RELEASED by owner ruling 2026-08-13 and migrated — now `src/features/papers/`; the rollout flags are still at 0 and the rest of this clause stands**), `components/quiz/`, `components/games/`, `AssessmentStudio.jsx` + `teacher/studio/` (**see the measured boundary below — this entry means the ASSESSMENT half of that directory, not all of it**) + `views/PaperBlocks|AssessmentPaperView`, `admin/PastPaperStudio`, `admin/CreateQuizV2`, and `DailyExamRunner`. Four of those are named on `scripts/visual/printAffectingPaths.js` by exact path, so they also carry the ~30-minute render gate — a second reason they go last rather than first.
