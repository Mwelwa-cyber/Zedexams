...
- Supporting server-only collections (all `write: if false` to clients): `aiUsage`, `aiDailyLimits`, `aiOperations`, `rateLimits`, `processedEvents`, `downloadTickets`, `paymentLocks`, `referralRedemptions`, `webauthnChallenges`, `deletionRequests`, `opsBackups`, `visitorStats`.
...
**Wave 4 — `classTimetable`.** The studio (setup wizard, drag-and-drop workspace, conflict panel, photo upload), the renderer, and three exporters into the engine — 21 relocated, 16 still in `src/utils/`. One exported name, `ClassTimetableView`.

**The timetable domain model stayed in `src/utils/`, and that is the recorded rule rather than an exception.** Five modules fail the "is the feature its only consumer" test: `classTimetable.js` (`data/studioSamples.js` reads it), `timetableGridModel.js` and `timetablePrintTemplates.js` (all three exporters build through them, and an engine may not import from a feature), and `timetableBlocks.js` / `timetableSessions.js` (`classTimetable.js` imports them, so they cannot sit above it). `src/utils/` is below both the feature and the engine, so nothing is inverted. This is deliberately NOT the `sbaTaskToPaper.js` treatment: those were single modules, and `src/shared/utils` is declared for "cross-cutting building blocks with no domain of their own" — a five-module timetable domain model is not that, and putting it there would make the shared layer mean something looser than its own index says. Four modules with only feature consumers did travel.

**A migration cleared a legacy debt line by creating a front door that did not exist.** `ClassTimetableStudio` imported `features/teacherSettings/lib/useTeachingProfile` — a *warning* while the studio was legacy, an *error* the moment it became a feature, because cross-feature imports must go through an index. `src/features/teacherSettings/index.js` did not exist. The contract's own rule is that a migration which would grow a debt list is telling you to fix the design, so the front door was created with the one name consumed across a boundary, the studio re-pointed, and the legacy line deleted: **9 → 8**. The four remaining legacy importers are legacy files and clear when their own callers migrate.

**The acknowledgement in `check:bundle-edges` was re-based on the causal EDGE, because the first-hop form churned three times in three migrations.** It was `SbaTaskView`, then `sba` when that feature got a front door, then `classTimetable` when Rollup merged the two front doors into one chunk — three rewrites, none of which changed what any page downloads. A record every unrelated migration has to edit stops being read. Entries now record `cut`: the edges into the vendor the exception rests on (`figureLabelLayout`, `quizRichText`, `migration` → `buildExtensions` — the renderer cluster's three routes to the editor schema). Each must exist, and with all of them removed the vendor must be unreachable, so a genuinely new dependency still fails. Chunk NAMES for feature front doors move as Rollup regroups; the edge into the vendor does not.
