...
export default function DiagramFixupPanel({ sections, subject, questionNumbers, onAttach, onAttachOption, onClose }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (cancelled) return
      setBank(rows)
      // Bounded concurrency, not one-at-a-time: this used to `await` each
      // picture's Storage round trip serially, so a bank of a few hundred
      // untagged pictures took one round trip's worth of latency PER
      // PICTURE (tens of seconds) before the last thumbnail ever appeared.
