 ...
 * options/stems carrying math came out blank or as raw `\frac{1}{3}` markup.
 *
 * The conversion itself lives in latexToUnicode.js, which PARSES rather than
 * pattern-matches. The ordered-regex version this replaced lost the bar of any
 * fraction whose numerator contained braces — the quadratic formula printed as a
 * product — and had no `\ce{}` handling at all, so chemistry printed without its
 * subscripts and mhchem's precipitate marker printed as the letter "v".
 *
 * Kept as a named export here because every caller and test already imports it
 * from this module.
 */
