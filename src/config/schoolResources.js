/**
 * School resource levels — how well-equipped the teacher's school is.
 *
 ...
 *
 * Client mirror of functions/teacherTools/schoolResources.js — keep the
 * value / promptLines pairs in sync with that file (same pattern as
 * learningEnvironments.js).
 */

export const SCHOOL_RESOURCE_LEVELS = [
  {
    value: 'low',
...
/** Default level — most Zambian classrooms sit here, not at "well-resourced". */
export const DEFAULT_SCHOOL_RESOURCES = 'basic'
...
export function schoolResourcePromptLines(value) {
...
export function schoolResourceLabel(value) {
  const level = BY_VALUE.get(String(value || '').toLowerCase())
