...
function renderMetaTable(meta) {
  ...
  else if (meta.teacherName) rows.push(['Teacher', esc(meta.teacherName)])
...
function renderOfficialHeader(meta) {
  const pairs = []
  const teacherDisplay = (meta.teacherName || meta.teacher || '') + (meta.tsno ? ' (TS ' + meta.tsno + ')' : '')
...
function renderOldModern(data, meta) {
  ...
  const headerPairs = [
    ['NAME', esc((meta.teacherName || meta.teacher || '') + (meta.tsno ? ' (TS ' + meta.tsno + ')' : ''))],
    ['DATE', esc(meta.date || '')],
