...
export default function LessonPlanStudio() {
  ...
  const handleGenerate = useCallback(async (lessonIndex = 0) => {
    ...
    try {
      ...
      // This runs in its OWN try/catch: the plan is already rendered and marked
      // 'done' above, so a Firestore write failure (offline, permission denied,
      // quota) must NOT flip the status to 'error' and hide a plan the teacher
      // successfully generated — it only logs.
      if (planningMode === 'series' && uid && effectiveSeriesId) {
        try {
          ...
          console.error('Studio: failed to persist series progress', persistErr)
        }
      }
    } catch (err) {
