// How an assessment paper is NAMED ON SCREEN — the library card and the
// builder's document title bar.
//
// This is deliberately a third string alongside the two in assessmentTitle.js:
//
...
//   • this module                                  — the name a teacher READS
//     in the studio and the library. Sentence-cased, and composed from the
//     paper's own fields so it can degrade by priority instead of by ellipsis.
//
...
// (scripts/test-paper-display-title.mjs).
...
/**
 * The library card's title. Always carries the SUBJECT, and always states the
 * term the paper's own `term` field states — the two facts whose absence made
 * seven different papers read as one name.
 *
 * "Grade 4 Integrated Science — End of Term 1 Test"
 *
 * A teacher's own name for the paper wins outright; the facts then ride on the
 * card's chip row beneath it.
 */
export function paperCardTitle(paper = {}, { fallbackYear } = {}) {
  const f = paperDisplayFacts(paper, { fallbackYear })
  if (f.customName) return f.customName
  const left = f.levelSubject || f.level
  const parts = [left, f.type].filter(Boolean)
  const base = parts.length === 2 ? `${parts[0]} — ${parts[1]}` : parts[0] || 'Untitled assessment paper'
  return f.paperName ? `${base} · ${f.paperName}` : base
}
