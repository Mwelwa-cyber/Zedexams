...
// Hoisted so the B-1 tests can hand the studio a real saved profile: the
// pre-branding it triggers is the input to that bug, not incidental setup.
const getSchoolProfileMock = vi.hoisted(() => vi.fn().mockResolvedValue(null))
vi.mock('../../utils/schoolProfileService', () => ({ getSchoolProfile: getSchoolProfileMock }))

// The cross-device draft store. Left real, the studio's mount would read
// localStorage and reach for Firestore, and the B-1 tests would be timing a
// network round-trip rather than the autosave gate.
const loadAssessmentDraftMergedMock = vi.hoisted(() =>
  vi.fn().mockResolvedValue({ draft: null, source: 'none' }))
const saveAssessmentDraftMock = vi.hoisted(() => vi.fn())
vi.mock('../../hooks/useAssessmentDraft', () => ({
  clearAssessmentDraft: vi.fn(),
  clearAssessmentDraftRemote: vi.fn(),
  loadAssessmentDraftMerged: loadAssessmentDraftMergedMock,
  saveAssessmentDraft: saveAssessmentDraftMock,
  saveAssessmentDraftRemote: vi.fn(),
}))
...
/*
 * B-1 — opening the studio must never file a paper in the library.
 *
 * Reported: sign in as a teacher, open /teacher/assessment-papers/new, type
 * NOTHING, and within a few seconds the header pill flips to "Saved to
 * library". Leave, and the papers list has gained a junk paper with one
 * question that has no text.
 *
 * The chain is entirely internal — no user input anywhere in it:
 *
 *   1. the studio seeds one empty starter question (a UI affordance, so the
 *      builder is not a blank rectangle)
 *   2. the saved School Profile loads and pre-brands the paper —
 *      applySchoolProfileDefaults writes form.schoolName
 *   3. the dirty effect re-runs on the form change. Its guard is
 *      "only-empty-starter AND no title AND no schoolName", and step 2 just
 *      filled in schoolName, so the guard no longer holds → libraryDirty
 *   4. shouldAutosaveToLibrary sees questionCount 1 (the empty starter counts)
 *      and libraryDirty true → persistAssessment files the paper
 *
 * So the app branded the paper on the teacher's behalf and then read its own
 * branding as an edit. The tests below pin both halves: the seeded question
 * is not content, and a programmatic form change is not an edit.
 */
describe('AssessmentStudio — B-1: opening the studio files nothing', () => {
  beforeEach(() => {
    mockParams = {}                       // /teacher/assessment-papers/new
    mockCreateAssessment.mockResolvedValue('new-paper-id')
  })

  it('does not create a library paper when the school profile pre-brands a fresh paper', async () => {
    getSchoolProfileMock.mockResolvedValue({
    ...
    render(<MemoryRouter><AssessmentStudio /></MemoryRouter>)
    await waitFor(() => expect(getSchoolProfileMock).toHaveBeenCalled())

    // The library autosave is on a 2s debounce. Wait past it — a test that
    // finishes before the timer fires would pass on a broken build.
    await new Promise((resolve) => setTimeout(resolve, 2600))

    expect(mockCreateAssessment).not.toHaveBeenCalled()
    expect(mockSaveAssessmentQuestions).not.toHaveBeenCalled()
  }, 10000)

  it('does not create a library paper with no school profile either', async () => {
    getSchoolProfileMock.mockResolvedValue(null)
  ...
  it('does not write a cross-device draft for an untouched, pre-branded paper', async () => {
    // The same "the app edited it, not the teacher" reasoning applies to the
    // draft store, which shares the guard. A draft written here comes back as
    // "Restored your unsaved draft" on the next visit, for work nobody did.
    getSchoolProfileMock.mockResolvedValue({ schoolName: 'Kabulonga Primary School' })

    render(<MemoryRouter><AssessmentStudio /></MemoryRouter>)
    await waitFor(() => expect(getSchoolProfileMock).toHaveBeenCalled())
    await new Promise((resolve) => setTimeout(resolve, 1400))

    expect(saveAssessmentDraftMock).not.toHaveBeenCalled()
  }, 10000)
})
