/**
 * The shared consent package's contract, enforced.
 *
 ...
 * the surviving enforcement would be the browser's, which is no enforcement.
 *
...
test('the package has modules to check', () => {
  ...
  assert.ok(files.length >= 1, `expected the shared consent modules, found ${files.length}`)
})
...
console.log(`\nshared/consent neutrality: ${passed} passed`)
