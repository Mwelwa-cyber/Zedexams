...
SCIENCE NOTATION — the same rule as maths: write it the way it should PRINT. This applies to EVERY field that can carry science — question text, options, the answer, the marking guide, sub-question parts and their answers, statements, table cells and any passage:
- Chemical formulae: write the count as a SUBSCRIPT using <sub></sub> — H<sub>2</sub>O, CO<sub>2</sub>, C<sub>6</sub>H<sub>12</sub>O<sub>6</sub>, H<sub>2</sub>SO<sub>4</sub>. NEVER write "H2O" or "CO2".
- A COEFFICIENT (how many molecules) stays full size and is NOT subscripted: 6CO<sub>2</sub>, 2H<sub>2</sub>O.
- Charges and ions use a SUPERSCRIPT: Na<sup>+</sup>, Cl<sup>-</sup>, SO<sub>4</sub><sup>2-</sup>.
- Equations use a REAL arrow →, never "->" or "=>". A reversible reaction uses ⇌. Word equations are first class and are what primary science uses: "carbon dioxide + water → glucose + oxygen".
- Units with an exponent use a superscript: m/s<sup>2</sup>, cm<sup>3</sup>, km<sup>2</sup>. Temperature uses the degree sign: 30 °C, never "30 deg C" or "30 degrees C".
...
Examples — area of a parallelogram: {kind:"shape", libraryKey:"parallelogramh", params:{base:"12 cm", height:"4 cm"}}. Volume of a box: {kind:"shape", libraryKey:"cuboid", params:{l:"10 cm", w:"3 cm", h:"2 cm"}}. Circumference of a circle: {kind:"shape", libraryKey:"circle", params:{radius:"6 cm", center:"O"}}. "Which shape is a regular hexagon?": {kind:"shape_options", options:[{libraryKey:"square"},{libraryKey:"rhombus"},{libraryKey:"hexagon"},{libraryKey:"pentagon"}]}, answer "C". "How many elements are in A∩B?": {kind:"shape", libraryKey:"vennelements", params:{a:"A", b:"B", onlyA:"1,2,3", both:"4,5", onlyB:"6,7,8", outside:"9"}}. Telling the time: {kind:"shape", libraryKey:"clockface", params:{hour:"3", minute:"15"}}. Reading an angle: {kind:"shape", libraryKey:"protractor", params:{angle:"60"}}. A graph the learner reads values off: {kind:"shape", libraryKey:"linegraph", params:{labels:"1,2,3,4,5,6,7", values:"40,50,20,60,30,40,30", cap:"Books per grade"}}. Integer arithmetic "Work out -3 + 5 using the number line": {kind:"shape", libraryKey:"numberlinejump", params:{min:"-5", max:"5", jumps:"-3>2"}}. Secondary examples — circle theorem "Find angle ABC": {kind:"shape", libraryKey:"circletheorem", params:{points:"A@200,B@90,C@340", joins:"O-A,O-C,A-B,B-C", angles:"AOC=110,ABC=x"}}. A bearings journey: {kind:"shape", libraryKey:"bearings", params:{legs:"A>B,060,8;B>C,135,6", unit:"km"}}. "Draw the graph of y = x² − 2x − 3 and use it to solve x² − 2x − 3 = 0": {kind:"shape", libraryKey:"functiongraph", params:{fn:"y = x^2 - 2x - 3", xMin:"-3", xMax:"5", yMin:"-6", yMax:"8", show:"roots,turning"}}. A rotation: {kind:"shape", libraryKey:"transformation", params:{object:"(1,1),(4,1),(1,3)", type:"rotation", by:"90,0,0", objectLabel:"P", imageLabel:"Q"}}. Reading a median off a curve: {kind:"shape", libraryKey:"ogive", params:{boundaries:"0,10,20,30,40,50", frequencies:"4,9,14,8,3", readOff:"median,quartiles"}}. A feasible region: {kind:"shape", libraryKey:"linearprogramming", params:{constraints:"x+y<=6,2x+y<=8,x>=0,y>=0", shade:"unwanted", regionLabel:"R"}}. Distance between two points on the same parallel: {kind:"shape", libraryKey:"earthgeometry", params:{points:"P:60N,20E;Q:60N,80E"}}.
...
7. Every figure you requested uses a libraryKey from the list above with only that key's params, and the params are ones the figure can actually be drawn from: angles that exist, three-figure bearings from 000 to 359, one frequency per class, a transformation image that fits inside the grid you set, inequalities with a region in common. A figure whose params cannot produce a true drawing BLOCKS the whole paper from being exported.`;
