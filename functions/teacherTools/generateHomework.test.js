/**
 * Node test for the generateHomework orchestration (runHomework in
 * functions/teacherTools/generateHomework.js). Focuses on the idempotency +
 ...
 * generateAssessment.test.js). aiOperationsCore (isValidIdempotencyKey) and
 * learningEnvironments load for real; homeworkSchema / homeworkPrompt are
 ...
 *
 * Run: node functions/teacherTools/generateHomework.test.js
 */
...
const {runHomework, createGenerateHomework} = require("./generateHomework");
...
(async function run() {
  ...
  reset();
  {
    const res = await runHomework({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ok("valid: returns generationId + homework", res.generationId && res.homework);
    ...
    ok("valid: reserved before provider", calls.reserve.length === 1 && calls.reserve[0].operationType === "generate_homework");
    ok("valid: settled once as completed", calls.complete.length === 1 && calls.complete[0].usageCharged === 1);
    ok("valid: usage charged exactly once", calls.meter.length === 1);
  ...
  reset();
  {
    ...
    let threw = null;
    try {
      await runHomework({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    } catch (e) {
      threw = e;
    }
    ...
    ok("throw: quota refunded (AI-006)", calls.refund.length === 1 && calls.refund[0].tool === "homework");
  ...
  reset();
  {
    ...
    const res2 = await runHomework({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ...
    ok("resume: returns the saved homework", res2.generationId === IDK && !!res2.homework);
  }

  // 4. Legacy caller (no key) → works; reservation untouched; failure still refunds.
  reset();
  {
    const res = await runHomework({uid: "u1", rawInputs: INPUTS, apiKey: "k"});
    ok("legacy: succeeds without a key", res.generationId && res.homework);
    ...
    let threw = null;
    try {
      await runHomework({uid: "u1", rawInputs: INPUTS, apiKey: "k"});
    } catch (e) {
      threw = e;
    }
  ...
  reset();
  {
    ...
    ok("flagged: no refund (teacher got a paper)", calls.refund.length === 0);
  }
  ...
  reset();
  {
    let threw = null;
    try {
      await runHomework({uid: "u1", rawInputs: {grade: "", subject: "", topic: ""}, apiKey: "k", idempotencyKey: IDK});
    } catch (e) {
      threw = e;
    }
  ...
  reset();
  {
    completeThrows = true;
    const res = await runHomework({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ok("settle-failure: still returns the homework", !!(res.generationId && res.homework));
  ...
  reset();
  {
    const handler = createGenerateHomework("secret");
  ...
  console.log(`\nAll generateHomework tests passed (${passed} assertions).`);
})().catch((err) => {
  Module._load = origLoad;
  console.error(err);
  process.exit(1);
