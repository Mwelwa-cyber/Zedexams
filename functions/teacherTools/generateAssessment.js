...
const {
  answerChoiceDirective, applyAnswerChoiceCount, normalizeRequestedCount,
...
function sanitizeInputs(raw = {}) {
  ...
  return {
    ...
    // §3 — how many answer choices every multiple-choice question offers.
    // null when the client did not say, in which case the generator's output is
    // left exactly as the model wrote it: imposing a count on a caller that
    // never chose one would silently reshape papers generated by older clients.
    answerChoiceCount: normalizeRequestedCount(raw.answerChoiceCount),
  };
}
...
async function runAssessment({uid, rawInputs, apiKey, idempotencyKey}) {
  ...
  const choiceDirective = answerChoiceDirective(inputs.answerChoiceCount);
  ...
  // §3 — the answer-choice count is ENFORCED, not merely requested. The prompt
  // above states it as a hard requirement, which the model mostly honours; this
  // is the half that does not depend on the model. A surplus option is trimmed
  // (keeping the correct one, never `slice(0, 4)` — that deletes the answer
  // whenever the model put it last), and a SHORTFALL is reported rather than
  // padded: a distractor invented here would be a wrong answer with no thought
  // behind it, printed on a real test, and nothing downstream would know it was
  // ours. The export gate blocks the short question and names it.
  const choiceCountResult = applyAnswerChoiceCount(assessment, inputs.answerChoiceCount);
  ...
  // §3 — what the count enforcement actually did. Surfaced rather than silent:
  // a trim that happened invisibly looks identical to a model that got it right,
