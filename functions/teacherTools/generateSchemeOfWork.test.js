...
const {runSchemeOfWork, createGenerateSchemeOfWork} = require("./generateSchemeOfWork");
...
(async function run() {
  ...
  reset();
  {
    for (const badKey of [undefined, null, "", "not-a-uuid", "   ", 12345,
      ...
      let threw = null;
      try {
        await runSchemeOfWork({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: badKey});
      } catch (e) {
        threw = e;
      }
  ...
  reset();
  {
    let threw = null;
    try {
      await runSchemeOfWork({uid: "u1", rawInputs: {grade: "", subject: "", topic: ""}, apiKey: "k", idempotencyKey: IDK});
    } catch (e) {
      threw = e;
    }
  ...
  reset();
  {
    completeThrows = true;
    const res = await runSchemeOfWork({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ok("settle-failure: still returns the scheme", !!(res.generationId && res.schemeOfWork));
  ...
  reset();
  {
    const handler = createGenerateSchemeOfWork("secret");
