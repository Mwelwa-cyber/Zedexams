...
const {runWorksheet, createGenerateWorksheet} = require("./generateWorksheet");
...
(async function run() {
  ...
  reset();
  {
    let threw = null;
    try {
      await runWorksheet({uid: "u1", rawInputs: {grade: "", subject: "", topic: ""}, apiKey: "k", idempotencyKey: IDK});
    } catch (e) {
      threw = e;
    }
  ...
  reset();
  {
    completeThrows = true;
    const res = await runWorksheet({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ok("settle-failure: still returns the worksheet", !!(res.generationId && res.worksheet));
  ...
  reset();
  {
    const handler = createGenerateWorksheet("secret");
