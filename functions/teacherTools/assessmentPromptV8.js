/**
 * Assessment prompt — v8.
 *
 * v7 → v8: integrated "Numeracy" balance. At lower primary (ECE + Grades 1-3)
 * the CBC bundles Mathematics and Science into ONE learning area whose subject
 * key is "numeracy" — and the syllabus hides the maths inside thematic topics
 * (e.g. "Exploring Materials" carries the "Plane and Solid shapes" sub-topic;
 * "Exploring Mixtures" carries "Addition and Subtraction of things"). v7 passed
 * the bare subject "numeracy" to the model with no hint it was a combined area,
 * so a Grade 1 "Maths & Science" paper came back science-only — body parts,
 * plants, hygiene — with no number, shape, time or measurement work at all. v8
 * adds an explicit requirement, fired ONLY for the numeracy subject, to cover
 * BOTH the number/maths strand AND the science/environment strand in roughly
 * equal measure.
 *
 * Everything else is carried over from v7 verbatim: the chosen-types whitelist
 ...
 * comprehension passages and matching questions. The schema is unchanged
 * (assessmentSchema v1.4).
 *
 * Still grounded on the verified <curriculum_module> context block when one
 * is present. When iterating, COPY this file to v9 rather than editing v8
 * in place.
 */
...
const PROMPT_VERSION = "assessment.v8";
