/**
 * The APPROVED lesson-plan system prompts. Server-owned, deliberately.
 *
 * These used to live in the browser (src/components/teacher/studio/utils/
 * studioSystemPrompt.js) and travel to `studioGenerateLessonPlan` as a
 * `systemPrompt` string the client chose. That made the generation contract
 * client-controlled: anything that could call the callable could replace the
 * curriculum instructions, the JSON contract and the writing standards with
 * its own text and still be metered, billed and stored as a ZedExams lesson
 * plan. Grounding a plan in the CBC knowledge base means nothing if the
 * instructions telling the model to use it can be swapped out in transit.
 *
 * The client now sends TYPED options. `curriculumMode` selects between these
 * two constants here, on the server, and nothing else can.
 *
 * The one intentional divergence from functions/teacherTools/lessonPlanPrompt.js
 * is preserved from the client copy: the studio exposes a Writing Style toggle,
 * so the WRITING STANDARDS block defers register and vocabulary to the style
 * named in the user prompt instead of hard-pinning "professional". Without that
 * the three styles all produced the same output and the toggle did nothing.
 */

const STUDIO_SYSTEM_PROMPT_CBC = `You are an expert Zambian teacher and CDC (Curriculum Development Centre) curriculum specialist. You write Competence-Based Curriculum (CBC) lesson plans that match the SAMPLE LESSON PLAN appendix of the official CDC teaching modules exactly as a Zambian head teacher or School Inspector would expect to see them.
...
const STUDIO_SYSTEM_PROMPT_PREVIOUS = `You are an expert Zambian teacher and curriculum specialist writing lesson plans aligned to the 2013 Previous Curriculum (Outcomes-Based Education). You write professional lesson plans that a Zambian head teacher or School Inspector would approve.
