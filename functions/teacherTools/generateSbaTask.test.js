/**
 * Node test for the generateSbaTask orchestration (runSbaTask in
 * functions/teacherTools/generateSbaTask.js). Focuses on the idempotency +
 * refund wiring added under the Phase 6 REL-001 migration (AI-002/AI-006):
 *
 ...
 * generateWorksheet.test.js). aiOperationsCore (isValidIdempotencyKey) loads
 * for real; sbaTaskSchema / sbaTaskPrompt / sbaTaxonomy are stubbed so this
 * test controls valid/invalid/throw deterministically without coupling to
 * their internals (each has its own test).
 *
 * Run: node functions/teacherTools/generateSbaTask.test.js
 */
  ...
  collection: (name) => {
    ...
    return {
      doc: (suppliedId) => {
        ...
        return {
          id,
          // SBA reserves with `.set(data)` then transitions with `.update()`.
          set: async (data, opts) => {
            if (opts && opts.merge) {
...
Module._load = function (request, ...rest) {
  ...
  if (request === "./sbaTaskSchema") {
    ...
    return {
      validateSbaTask: (parsed) => {
        const okFlag = !!(parsed && parsed.__valid !== false);
        return {ok: okFlag, value: parsed || {}, errors: okFlag ? [] : ["schema error"]};
      },
    };
  }
  if (request === "./sbaTaskPrompt") {
    return {
      PROMPT_VERSION: "sba-test",
      SYSTEM_PROMPT: "SYSTEM",
      buildUserPrompt: () => "USER",
    };
  }
  if (request === "./sbaTaxonomy") {
    return {
      SUBJECT_LABELS: {english: "English"},
      GRADE_LABELS: {G5: "Grade 5"},
      CTS_COMPONENT_LABELS: {},
      BLOOM_LEVELS: new Set(["Comprehension"]),
      isSbaSubject: () => true,
      isSbaGrade: () => true,
      resolveSbaTaskMeta: () => ({
        defaultMarks: 20,
...
const {runSbaTask, createGenerateSbaTask} = require("./generateSbaTask");

const INPUTS = {grade: "G5", subject: "english", taskType: "reading_comprehension", topic: "The environment"};
...
(async function run() {
  ...
  reset();
  {
    const res = await runSbaTask({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ok("valid: returns generationId + task", res.generationId && res.task);
    ...
    ok("valid: reserved before provider", calls.reserve.length === 1 && calls.reserve[0].operationType === "generate_sba_task");
    ok("valid: settled once as completed", calls.complete.length === 1 && calls.complete[0].usageCharged === 1);
    ok("valid: usage charged exactly once", calls.meter.length === 1 && calls.meter[0].tool === "sba_task");
  ...
  reset();
  {
    ...
    let threw = null;
    try {
      await runSbaTask({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    } catch (e) {
      threw = e;
    }
    ...
    ok("throw: quota refunded (AI-006)", calls.refund.length === 1 && calls.refund[0].tool === "sba_task");
  ...
  reset();
  {
    ...
    const res2 = await runSbaTask({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ...
    ok("resume: returns the saved task", res2.generationId === IDK && !!res2.task);
  }
  ...
  reset();
  {
    for (const badKey of [undefined, null, "", "not-a-uuid", "   ", 12345,
      ...
      let threw = null;
      try {
        await runSbaTask({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: badKey});
      } catch (e) {
        threw = e;
      }
  ...
  reset();
  {
    ...
    ok("flagged: no refund (teacher got a task)", calls.refund.length === 0);
  }

  // 6. Invalid inputs → invalid-argument before any provider/reservation work.
  reset();
  {
    // isSbaGrade/isSbaSubject/resolveSbaTaskMeta are stubbed to accept, so drive
    // the rejection through them for this one case.
    const origLoad2 = Module._load;
    let threw = null;
    // Temporarily reject validation by swapping the taxonomy stub.
    Module._load = function (request, ...rest) {
      if (request === "./sbaTaxonomy") {
        return {
          SUBJECT_LABELS: {}, GRADE_LABELS: {}, CTS_COMPONENT_LABELS: {},
          BLOOM_LEVELS: new Set(),
          isSbaSubject: () => false,
          isSbaGrade: () => false,
          resolveSbaTaskMeta: () => null,
          ctsComponentToKbSubject: (c) => c,
        };
      }
      return origLoad2.call(this, request, ...rest);
    };
    delete require.cache[require.resolve("./generateSbaTask")];
    const {runSbaTask: runReject} = require("./generateSbaTask");
    try {
      await runReject({uid: "u1", rawInputs: {grade: "X", subject: "y", taskType: "z"}, apiKey: "k", idempotencyKey: IDK});
    } catch (e) {
      threw = e;
    }
    ...
    // Restore the accepting stub for the remaining cases.
    Module._load = origLoad2;
    delete require.cache[require.resolve("./generateSbaTask")];
  }

  // Re-require the module under the accepting stub for the rest.
  const {runSbaTask: runOk, createGenerateSbaTask: createOk} = require("./generateSbaTask");
  ...
  reset();
  {
    ...
    const res = await runOk({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
  ...
  reset();
  {
    ...
    let threw = null;
    try {
      await runOk({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    } catch (e) {
      threw = e;
    }
  ...
  reset();
  {
    completeThrows = true;
    const res = await runOk({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ok("settle-failure: still returns the task", !!(res.generationId && res.task));
  ...
  reset();
  {
    ...
    await runOk({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    const claudeAfterFirst = calls.claude.length;
    const res2 = await runOk({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
  ...
  console.log(`\nAll generateSbaTask tests passed (${passed} assertions).`);
})().catch((err) => {
  Module._load = origLoad;
  console.error(err);
  process.exit(1);
