/**
 * Node test for the generateFlashcards orchestration (runFlashcards in
 * functions/teacherTools/generateFlashcards.js). Focuses on the idempotency +
 ...
 * real; flashcardSchema / flashcardPrompt are stubbed so this test controls
 * valid/invalid/throw deterministically without coupling to their internals
 * (each has its own test).
 *
 * Run: node functions/teacherTools/generateFlashcards.test.js
 */
...
Module._load = function (request, ...rest) {
  ...
  if (request === "./flashcardPrompt") {
    return {
      PROMPT_VERSION: "fc-test",
...
(async function run() {
  ...
  reset();
  {
    ...
    ok("valid: returns generationId + flashcards", res.generationId && res.flashcards);
    ...
    ok("valid: reserved before provider", calls.reserve.length === 1 && calls.reserve[0].operationType === "generate_flashcards");
  ...
  reset();
  {
    ...
    ok("throw: quota refunded (AI-006)", calls.refund.length === 1 && calls.refund[0].tool === "flashcards");
  ...
  reset();
  {
    ...
    ok("resume: returns the saved flashcards", res2.generationId === IDK && !!res2.flashcards);
  }

  // 4. Legacy caller (no key) → works; reservation untouched; failure still refunds.
  reset();
  {
    const res = await runFlashcards({uid: "u1", rawInputs: INPUTS, apiKey: "k"});
    ok("legacy: succeeds without a key", res.generationId && res.flashcards);
    ...
    let threw = null;
    try {
      await runFlashcards({uid: "u1", rawInputs: INPUTS, apiKey: "k"});
    } catch (e) {
      threw = e;
    }
  ...
  console.log(`\nAll generateFlashcards tests passed (${passed} assertions).`);
})().catch((err) => {
  Module._load = origLoad;
  console.error(err);
  process.exit(1);
