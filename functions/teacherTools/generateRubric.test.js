/**
 * Node test for the generateRubric orchestration (runRubric in
 * functions/teacherTools/generateRubric.js). Focuses on the idempotency +
 ...
 *      teacher got a rubric), no refund.
 *
 ...
 * real; rubricSchema / rubricPrompt are stubbed so this test controls
 * valid/invalid/throw deterministically without coupling to their internals
 * (each has its own test).
 *
 * Run: node functions/teacherTools/generateRubric.test.js
 */
...
Module._load = function (request, ...rest) {
  ...
  if (request === "./rubricPrompt") {
    return {
      PROMPT_VERSION: "rubric-test",
...
const {runRubric, createGenerateRubric} = require("./generateRubric");
...
(async function run() {
  ...
  reset();
  {
    const res = await runRubric({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ok("valid: returns generationId + rubric", res.generationId && res.rubric);
    ...
    ok("valid: reserved before provider", calls.reserve.length === 1 && calls.reserve[0].operationType === "generate_rubric");
    ok("valid: settled once as completed", calls.complete.length === 1 && calls.complete[0].usageCharged === 1);
    ok("valid: usage charged exactly once", calls.meter.length === 1 && calls.meter[0].tool === "rubric");
  ...
  reset();
  {
    ...
    let threw = null;
    try {
      await runRubric({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    } catch (e) {
      threw = e;
    }
    ...
    ok("throw: quota refunded (AI-006)", calls.refund.length === 1 && calls.refund[0].tool === "rubric");
  ...
  reset();
  {
    ...
    const res2 = await runRubric({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ...
    ok("resume: returns the saved rubric", res2.generationId === IDK && !!res2.rubric);
  }

  // 4. Legacy caller (no key) → works; reservation untouched; failure still refunds.
  reset();
  {
    const res = await runRubric({uid: "u1", rawInputs: INPUTS, apiKey: "k"});
    ok("legacy: succeeds without a key", res.generationId && res.rubric);
    ...
    let threw = null;
    try {
      await runRubric({uid: "u1", rawInputs: INPUTS, apiKey: "k"});
    } catch (e) {
      threw = e;
    }
  ...
  reset();
  {
    ...
    ok("flagged: no refund (teacher got a rubric)", calls.refund.length === 0);
  }
  ...
  reset();
  {
    let threw = null;
    try {
      await runRubric({uid: "u1", rawInputs: {grade: "", subject: "", topic: ""}, apiKey: "k", idempotencyKey: IDK});
    } catch (e) {
      threw = e;
    }
  ...
  reset();
  {
    completeThrows = true;
    const res = await runRubric({uid: "u1", rawInputs: INPUTS, apiKey: "k", idempotencyKey: IDK});
    ok("settle-failure: still returns the rubric", !!(res.generationId && res.rubric));
  ...
  reset();
  {
    const handler = createGenerateRubric("secret");
  ...
  console.log(`\nAll generateRubric tests passed (${passed} assertions).`);
})().catch((err) => {
  Module._load = origLoad;
  console.error(err);
  process.exit(1);
