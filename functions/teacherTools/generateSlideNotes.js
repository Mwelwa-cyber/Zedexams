 ...
 *                     (OpenAI gpt-image-1) and write the resulting Storage URL
...
// gpt-image-1 medium is ~$0.06 per image; mirror that for the per-deck estimate.
const IMAGE_COST_CENTS = 6;

// Image generation tuning. gpt-image-1 round-trips (generate + download to
...
// gpt-image-1 tail + the upfront Claude call must all fit under the 300s
 ...
 */
async function enrichDeckImages({uid, deckId, deck, openaiKey}) {
  const targets = [];
  forEachImageTarget(deck, (t) => targets.push(t));

  // All illustrations render via ChatGPT / gpt-image-1 (full-colour, closer to
  // the reference decks). Recraft was decommissioned, so a missing OpenAI key
  // means no image provider at all.
  const provider = "openai";
  const haveKey = Boolean(openaiKey);
...
async function runSlideNotes({uid, rawInputs, apiKey, openaiKey}) {
...
function createGenerateSlideNotes(
    anthropicApiKeySecret, openaiApiKeySecret) {
  ...
  return onCall(
    ...
    async (request) => {
      ...
      await assertGeneratorRateLimit(request, "slide_notes");
      ...
      try {
        return await runSlideNotes({
          uid, rawInputs: request.data, apiKey, openaiKey,
        });
      } catch (err) {
