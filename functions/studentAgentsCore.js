/**
 * Pure decision logic for the learner-facing study-plan agent
 * (functions/studentAgents.js): result summarisation, fallback-plan
 * construction, AI-plan normalisation/validation, and prompt shaping.
 *
 * Constraint: this module must stay importable under plain `node` with no
 * firebase-admin / firebase-functions dependency (the *Core.js repo
 * convention), so it must NOT require ./aiService — that module pulls in
 * firebase-admin at load time. The two tiny pure helpers it needs from
 * there (cleanString, stripJsonFences) are therefore carried here.
 */
