...
2. **App Check on `apiTextToSpeech`** — the priciest per-call surface should join the soft-verify + graduated-enforcement set.
3. **Broaden `assertCallableRateLimit`** to the remaining metered AI generators (currently bounded by daily/monthly caps but not per-minute burst). Mechanical, fail-open.
